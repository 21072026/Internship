...
| **Accessibility (a11y)** | Landmarks, roles, keyboard/contrast basics, status messages (4.1.3), reflow at 320px; OS media preferences (reduced motion, increased contrast, forced colors) | `e2e/a11y.spec.ts`, `e2e/board-a11y.spec.ts`, `e2e/live-region.spec.ts`, `e2e/mobile-layout-audit.spec.ts`, `e2e/mobile-layout-coverage.spec.ts`, `e2e/a11y-media-preferences.spec.ts` | with E2E |
...
| **Responsive / mobile** | Layout at small viewports; the mechanical phone-width audit and its route-coverage gate | `e2e/mobile.spec.ts`, `e2e/users-responsive.spec.ts`, `e2e/mobile-layout-audit.spec.ts`, `e2e/mobile-layout-coverage.spec.ts`, `e2e/mobile-fixed-bars.spec.ts`, `e2e/mobile-chat-layout.spec.ts` | with E2E |
| **PWA / offline** | Manifest, service worker, offline page; and what the worker's cache may hold — no `/api/` entry, emptied on sign-out, registered for signed-out visitors (#1550) | `e2e/pwa.spec.ts`, `e2e/offline-page.spec.ts`, `e2e/sw-cache-scoping.spec.ts` (the one spec that runs with `serviceWorkers: 'allow'`), `scripts/test/service-worker-cache.test.mjs` | with E2E; the worker unit test on **every PR** |
...
| **2.5.8 Target Size (Minimum)** | axe's `target-size` rule does see it, but only on the pages `e2e/a11y-scan.spec.ts` visits, and only at that spec's desktop viewport — a control is cramped at 360px, not at 1280px. | `e2e/helpers/layoutAudit.ts`'s `tapTargets()`, run at 360px over every route in `e2e/mobile-layout-coverage.spec.ts`. It implements the criterion's **spacing exception** (a 24px circle centred on an undersized target may not reach another target), because without it the rule fails on every 16px checkbox in the product and stops being a finding. |
