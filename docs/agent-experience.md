...
**When two specs describe the identical symptom, check whether the helper they call already has the fix.** `signInAsFreshUser()` / `submitSignInForm()` in `e2e/helpers/auth.ts` already existed, already had a detailed doc comment about the exact "session cookie re-issued mid-navigation, `/auth/signin` sees `authenticated`, `router.replace()`s to the *previous* user's dashboard, submit button detaches" race — from an earlier, unrelated fix. Three of the six failing specs were just calling the *wrong* helper (`signInAndSettle`, meant for a first sign-in, not a same-page re-login) or hand-rolling the same broken pattern locally instead of importing the shared one. No new race-handling code was needed, only wiring existing helpers to the right call sites.

**A shared `settle()`/timeout symptom can hide two unrelated causes — fix the visible one, then look again before declaring victory.** `mobile-layout-audit.spec.ts`'s `/release-notes` test showed the exact same "element detached, retrying" login-race shape as the other five failures, but its actual bug was different: `settle()` waits for `account-menu-button`, which only exists on *authenticated* role shells, and `/release-notes` is a public page — so it was never going to succeed regardless of the login race. Fixing that (a `settlePublic()` that waits for the page's own `<h1>`) let the test actually reach its real assertion, which then failed for a third, genuine reason: a CSS overflow bug in the shared `PublicFooter` (German compound words don't wrap in a 148px-wide footer column at 360px). Two of the three layers were test-side; the third was real and got filed as #1486, not forced green. The instinct to stop at "timeout fixed" would have shipped a red test with a wrong root-cause commit message.

**A `getByTestId(...)` that never appears isn't always a timing race — check what has to render *before* it.** `person-hover-card.spec.ts`'s missing trigger looked like a data race (element genuinely never in the DOM, not just hidden), but the actual gate was a feature flag: opening a project's internal view is behind a contributor-terms acceptance check (#1026), and the test's freshly seeded project owner had never accepted it, so the whole page rendered the "accept terms" wall instead of the join-requests panel the trigger lives in. `e2e/helpers/db.ts` already had `acceptContributorTerms(userId, { projectId })` for exactly this from when #1026 shipped — one call fixed it, no waiting logic needed.

**Two locator-ambiguity bugs (#5/#6 in this batch) had the same shape: a page's own accessible name grew to substring-contain the thing a test was already matching by regex/substring.** `getByRole('link', { name: /home/i })` started also matching a header aria-label ("…go to the home page") once that label shipped; `getByLabel('Mentor')` started also matching a dialog's own name ("Assign mentorship") once dialog naming was added. Neither test was ever wrong when written — a *later, unrelated* feature made the loose match ambiguous. The fix in both cases was anchoring/exactness (`^...$` regex, or `{ exact: true }` against the field's real rendered label including its "*"), not scoping by container — worth checking the simpler fix first.

**`git checkout -b <new-branch> origin/main` with uncommitted changes in the working tree just works** as long as `origin/main` hasn't touched the same files since the branch you're leaving — Git carries the uncommitted diff onto the new base automatically. Cheaper than stashing/patching by hand when you've verified (`git diff HEAD origin/main -- <files>`) there's no overlap.
...
**Düzeltme (aynı gün, ayrı bir olaydan sonra):** #2509 doğrusu **duplikeydi**. `main`'e az
sonra inen #2507'nin kaydettiği üzere `resolved to 2 elements` bu repoda zaten bilinen, eski
ve yaygın bir aile (#2312/#2316/#2479 — son 15 zamanlanmış koşunun 9'unda, ~9 farklı spec'te);
"yerelde tekrarlanamadı" bu ailede karar sinyali değildir çünkü ırk koşulu genelde tek
worker'lı yerel bir çalıştırmada hiç tetiklenmiyor. #2509'u #2479'un (aynı sayfa, aynı
`AccountSettings` bileşeni) duplikesi olarak kapattım. Ders: "yerelde 4/4 yeşil" + "kapanmış
