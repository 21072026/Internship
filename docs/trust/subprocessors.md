...
| 7 | **8x8 public Jitsi instance (`meet.jit.si`)** | The default video room when JaaS is not configured, the fallback when a JaaS call cannot start, and where any meeting is held once the month's JaaS participant allowance is used up (#2011) | Display name, room identifier, live audio/video stream | 8x8's public instance | Public-service terms only — no contract | Used only if someone opens a meeting room; there is no way to configure it away while video is in use |
...
| 9 | **Plausible Analytics** (`NEXT_PUBLIC_PLAUSIBLE_DOMAIN`, `NEXT_PUBLIC_PLAUSIBLE_HOST`) | Pageview measurement on **public marketing pages only**, plus **one anonymous click event** (`demo_cta_click`) when a visitor opens the public demo — the demo runs on a separate origin, so without it a click is an unmeasured exit (#1391) | Pageview URL, referrer, coarse device and browser data. Cookieless. For the click event: the placement of the link that was used (`hero`, `cta` or `footer`) and nothing else — no identifier, no session and nothing about the person. It is sent through the provider script that consent already loaded, not through a beacon of our own (`src/lib/track.ts`) | Operator-chosen; defaults to `plausible.io`. Plausible is self-hostable, so an operator can keep this on their own infrastructure | Plausible's DPA, held by the operator — or none needed when self-hosted | **Optional + consent** — unset by default, and loads only after the visitor accepts analytics cookies |
| 10 | **PostHog** (`NEXT_PUBLIC_POSTHOG_KEY`, `NEXT_PUBLIC_POSTHOG_HOST`) | Pageview measurement on **public marketing pages only**, plus **one anonymous click event** (`demo_cta_click`) when a visitor opens the public demo — the demo runs on a separate origin, so without it a click is an unmeasured exit (#1391) | Pageview URL, referrer, coarse device and browser data. Autocapture, session recording and `localStorage` persistence are forced **off in code** (`src/lib/analytics.ts`), not left to a dashboard setting. For the click event: the placement of the link that was used (`hero`, `cta` or `footer`) and nothing else — no identifier, no session and nothing about the person. It is sent through the provider script that consent already loaded, not through a beacon of our own (`src/lib/track.ts`) | Operator-chosen; the shipped default host is `eu.i.posthog.com`, PostHog's EU region | PostHog's DPA, held by the operator | **Optional + consent** — as row 9 |
| 11 | **Google Analytics 4** (`NEXT_PUBLIC_GA4_MEASUREMENT_ID`) | Pageview measurement on **public marketing pages only**, loaded with `anonymize_ip`, plus **one anonymous click event** (`demo_cta_click`) when a visitor opens the public demo — the demo runs on a separate origin, so without it a click is an unmeasured exit (#1391) | Pageview URL, referrer, truncated IP, coarse device and browser data. For the click event: the placement of the link that was used (`hero`, `cta` or `footer`) and nothing else — no identifier, no session and nothing about the person. It is sent through the provider script that consent already loaded, not through a beacon of our own (`src/lib/track.ts`) | Google | Google's Analytics terms and standard contractual clauses, held by the operator | **Optional + consent** — as row 9, and unset in the reference deployment |
