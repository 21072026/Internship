...
## [0.214.3-beta] - 2026-09-23

_Shipped 2026-09-23 20:18 UTC · commit [6b0a0f1](https://github.com/21072026/Internship/commit/6b0a0f1d76c3f07559a94f93a4050a6140f8144a)_

- **The marketing product moved to its own domain** (#2540, epic #2348). `marketing.bcsit-gmbh.de` is the live SaleVali host and `marketing.bcsit-gmbh.dev` the test one, replacing `marketing.ersah.in` / `preview-marketing.ersah.in`; the retired names redirect. Only the *web* hosts moved — the `ersah.in` apex keeps doing mail (sending domain, SPF/DKIM/DMARC, inbound, reply and unsubscribe tokens, ICS organisers), and the internship hosts are untouched. In the app this is one constant: `DEFAULT_MARKETING_HOST` in `src/lib/servedHosts.ts` is read both by `hostVertical.ts` (which host shows the marketing landing) and by the #2488 redirect allowlist (which host a redirect may stay on), so the two cannot drift apart — and because prod deliberately configures no `MARKETING_HOSTS`, that constant *is* the production domain. The default is deliberately still a SINGLE host: listing the `.dev` test domain there would make it a valid redirect target on production. Preview overrides it with `MARKETING_HOSTS='marketing.bcsit-gmbh.dev'`. On the box each name is on a fresh apex with no wildcard, so Caddy issues its certificate automatically rather than reusing the `*.ersah.in` files (`.dev` is HSTS-preloaded, so that certificate has to exist before a browser will connect at all). A new unit assertion fails if the default ever lands back on `ersah.in`, and both marketing hosts joined the uptime probe — the only check that can see a dead certificate or a missing Caddy site file for a name that shares its containers with a healthy one. Because cookies, storage, the installed PWA and the service worker are all per-origin, a signed-in marketing user is signed out by the move and re-installs the app under the new name; push subscriptions made on the old origin keep delivering until they are replaced.

## [0.214.2-beta] - 2026-09-23

_Shipped 2026-09-23 20:06 UTC · commit [23a2eef](https://github.com/21072026/Internship/commit/23a2eefcbaa000afe0130fbb82a89ddac2687034)_

- **A MARKETING demo tenant behind the shipped funnel preset** (#2443, story #2398). `npm run seed:demo` seeded exactly one organization and never set `vertical`, so every demo row was an INTERNSHIP tenant and the `MARKETING_FUNNEL` stage preset (#2353) had nothing at all behind it — the marketing board, funnel, ageing and drop-off screens rendered perfectly with zero rows on every demo, topic environment and contributor's box. The seeder now adds a second organization (`demo-marketing`, `vertical: 'MARKETING'`, PRO) with the preset's own `PipelineStage` rows (LEAD_NEW … DEAL_LOST, trial pair included), a tenant admin, three account owners and thirty synthetic German merchant accounts on the existing `Company` model — each with a lead `User` and a `MentorshipRelation` funnel record per docs/marketing-vertical/pipeline-record.md, spread across all nine stages, with 94 back-dated `StatusChange` rows and 61 `InteractionLog` rows so stage ageing and the analytics curves have a shape to draw. New `prisma/seed-demo-marketing.mjs`; the stage list is a plain-ESM mirror of the preset because the seeder runs in the runtime image (node:20-slim, no `src/`), and `scripts/test/marketing-demo-tenant.test.mjs` fails the build if the mirror and `defaultTemplateForVertical('MARKETING')` ever disagree. The tenant admin and the three owners sign in with the usual demo password; the lead people carry `NO_LOGIN_PASSWORD` (the same sentinel the marketing importer writes), so they are records rather than thirty sign-in-able MENTEE accounts in a vertical that has no mentee portal. Six marketing-scoped entries added to `scripts/demo-fidelity.json`; the seeder stays idempotent and `assertSafeDemoTarget()` still refuses a non-local database.

## [0.214.1-beta] - 2026-09-23

_Shipped 2026-09-23 20:06 UTC · commit [3622bd0](https://github.com/21072026/Internship/commit/3622bd0aecf09672bd009b7415b5c0b249c4dcde)_

- **SaleVali usage feed: the data contract and its tables** (#2445, #2446). `docs/marketing-vertical/salevali-usage-feed.md` records the transport, the identity key, the fields and the refresh window; `Company.externalId` (nullable, indexed `[orgId, externalId]`, deliberately not `@unique`) and the new `CompanyUsage` model (`@@unique([companyId, date])`, cascade from `Company`) are the tables it fills. `CompanyUsage` is registered in `TENANT_MODELS` and has a 400-day retention entry. No job, endpoint or screen reads it yet (#2447).

## [0.214.0-beta] - 2026-09-23

_Shipped 2026-09-23 20:05 UTC · commit [4e9f0a0](https://github.com/21072026/Internship/commit/4e9f0a0a7a04da26250b3b2c69c60abefec1bd45)_

- **The company/account list sorts, pages and searches on the server, and says what a delete costs** (#2436, #2437, #2441, story #2397). `GET /api/companies` now takes `sort`, `search`, `page`/`pageSize` and returns `total` — the `/api/candidates` shape, no new pagination helper — while still reading through the `company` scope from #2431. Four orders: name A–Z (default), recently added, last stage movement (newest real `StatusChange`) and longest in stage (`daysInStage`); an unknown value falls back to the default instead of failing, and an account with **no** stage movement sorts LAST in both stage-derived orders rather than being treated as "moved longest ago" (`src/lib/companySort.ts`, unit-tested; `lastStageMoveAt()` added to `src/lib/stageClock.ts` so "a move" keeps meaning one thing). The client-side `companies.filter(...)` in `/admin/companies` is gone; the screens that render a company `<select>` (mentorship assignment, offers, project form) pass `all=1`. Deleting a company no longer goes through the browser's `confirm()`: a new `GET /api/companies/[id]/delete-impact` (ADMIN, tenant-scoped) counts the dependants and `ConfirmDialog` states them in two separate lines — what cascades away with the account (positions, entitlements, need alerts, candidate interests, requisitions, interview requests) and what survives with its company link cleared (mentorships, projects, offers, placements, company logins, company inquiries). `e2e/companies-list-workflow.spec.ts` covers nulls-last ordering, server-side search + paging and the dialog's consequence text.

## [0.213.0-beta] - 2026-09-23

_Shipped 2026-09-23 20:03 UTC · commit [177c071](https://github.com/21072026/Internship/commit/177c0714b7e32ecf93634303d4387443c8d70bf7)_

- **Cohort conversion and a retention triangle on the analytics page** (#2420, #2425, story #2393). Two pure functions next to `stageConversions()` in `src/lib/funnelKpi.ts`, both fed by the `Journey[]` the funnel route already builds from `StatusChange` rows. `conversionByEntryMonth()` groups a conversion by the month the record ENTERED the first stage rather than the month it converted — a deal that closes months later is credited to the month it arrived, which is what stops a month from reading above 100% (the numerator and denominator are now decided by the same `furthestIndex()` pass, so the excess is impossible rather than unlikely, and the cohort denominators add up to the `entered` column of the stage-to-stage card). `retentionTriangle()` cohorts by the month a record first reached the tenant's won stage and reports, per 1/3/6/12-month bucket, the share that later arrived at a loss stage; **a bucket whose window has not closed yet returns `null`, never `0`**, and renders as `—`. Churn is deliberately defined as "any off-path arrival after the winning arrival" rather than as a new CHURNED stage in the marketing preset: a preset stage would only exist for tenants provisioned after it landed and every org already created would report a permanent 0% churn. Stage keys are resolved per tenant through `outcomeStageKeysFrom()` (#1882) — no `DEAL_WON`/`CHURNED_900`/`CANCELLATION_NOTICE_800` literal anywhere — and month boundaries reuse the repo's single UTC half-open month (`periodOf`/`periodRange`). `GET /api/admin/analytics/funnel` returns both as `cohortConversion` and `retention`; the analytics page renders them as a new Cohorts card. Unit tests: `scripts/test/funnel-cohorts.test.mjs` (15 assertions, `npm run test:funnel-cohorts`), with a 90% line floor on `funnelKpi.ts`; `e2e/funnel-kpi.spec.ts` seeds a record won in the current month and asserts the open bucket renders as a dash on screen.

## [0.212.0-beta] - 2026-09-23

_Shipped 2026-09-23 20:03 UTC · commit [56bb0cd](https://github.com/21072026/Internship/commit/56bb0cd342d6f5ff181f511d6da2522d1d59ad1b)_

- **`trial-reminders`: the job that keeps a trial from running out in silence** (#2415, #2410, #2417, #2418, #2416, story #2392). New `src/lib/jobs/trialReminders.ts` (`runTrialReminders()` + `initTrialRemindersCron()`, registered from `/api/cron/start` at 05:20 UTC and runnable on its own through `GET /api/cron?job=trial-reminders`, optionally narrowed with `&orgId=`). The sweep is per tenant and gated by `verticalFor(orgId)` — a sessionless job binds its own org with `runWithOrg()` rather than inheriting one. The recipient is the record's `mentorId` (the account owner in MARKETING terms); a record parked on a deactivated account falls back to the ADMINs **of that org only**, never every admin on the platform. Both channels go out behind their own switch — `notificationCategoryAllowed(user, 'deadlines')` for the bell and `emailAllowed()` + `emailGroupAllowedForCategory()` for the mail, on the existing `stage-deadline` category with no new category and no new preference key — each send in its own try/catch, and the job returns `{ considered, sent, skipped, failed, expired, orgs }`. The bell's gate is applied from the prefs the job already holds rather than handed to `notifyIfAllowed()`, because that helper returns void: counting a call to it as a delivery counted a *muted* recipient as one, which made `skipped` unreachable and let a silent pair spend the sending budget. A channel that throws is counted in `failed` rather than only logged, so a broken mail path cannot report itself as a perfect run. **Baseline (#2410): a lower bound, not a backfill** — the selector is an exact calendar-day match, so an elapsed trial equals no threshold ever again, and `findDueTrialReminders` bounds `trialEndsAt` to a window two days either side of the ladder; the decision is recorded in the header of `prisma/backfill-cron-baseline.mjs` beside the existing entries, and a `TRIAL_REMINDER_MAX_PER_RUN = 50` ceiling (modelled on `DORMANT_NUDGE_MAX_PER_RUN`) drains the queue most-urgent-first so a capped tick sheds the marks the ladder can absorb. **Auto-advance (#2417):** a trial whose last day is past moves from the tenant's `TRIAL_ACTIVE` stage to `TRIAL_EXPIRED` through a conditional `updateMany`, so a human who moved the row first wins; `stageDeadlineUpdate()` sets the new stage's clock; the audit row is `AuditLog` + `actorId: 'system'` (as `expireOffers()` does) because `StatusChange.changedById` is a required FK to `User` and a system user would be a real account on somebody's seat count. `TRIAL_EXPIRED` stays `isOffPath: false`, so `validateDropoffReason()` never applies to an unattended move. The expiry pass is bounded at `TRIAL_EXPIRY_MAX_PER_RUN = 500` records a tick — its predicate stays true tomorrow, so a backlog (an imported customer base with historical trial dates) drains over a few ticks instead of in one long admin-triggered request. Known consequence of the `AuditLog` choice, tracked in #2527: because `StatusChange` is the only thing `src/lib/stageClock.ts` reads, an automatically expired record reports the age of the whole trial as its time in the new stage. **Attention queue (#2418):** a new `'trial_expired'` reason in `src/lib/mentorAttention.ts` lists records waiting for a decision, labelled from the dictionary in all three languages — no new board card and no second time-in-stage calculation (`src/lib/stageClock.ts` still owns that). `e2e/trial-reminders-job.spec.ts` seeds a MARKETING tenant with a trial exactly seven days out, calls the endpoint twice as ADMIN and asserts one `TrialReminder` row, one bell item, one expiry audit row and `sent: 0` on the second run — with no SMTP configured and no new mail transport mode.

## [0.211.0-beta] - 2026-09-23

_Shipped 2026-09-23 20:03 UTC · commit [629c77e](https://github.com/21072026/Internship/commit/629c77ef51f2b3e8ad431a6b2953acce5262e6ea)_

- **The last two public pages a marketing visitor could click through to now speak about the marketing product** (#2475, #2457, epic #2348). `/features` filtered its catalogue by the request vertical's capabilities — the same `caps.includes(f.capability)` expression the landing grid already used — and the still-untagged internship cards (mentee insights, intern projects, project teams and join requests, mentor self-apply, weekly reports, offers, requisitions, interview requests, talent pool, rematch, match quality, dormant check-in, stories, dual role, the career newsletter and the published price list) carry a capability at last; a card that holds for every vertical deliberately carries none, and the rule is written at the top of `src/lib/features.ts`. `/pricing` does NOT get invented marketing prices: every number on it comes from `src/lib/plans.ts` and prices the internship product (the metering unit is the active matched pair), and the marketing packaging has not been decided, so a MARKETING host hides the free core, the plan bands, the meter, the overage, the employer wallet, the add-ons and the four internship CTAs, keeping only the two claims that hold without a price list (self-hosting is free under AGPL-3.0; there is no checkout, a paid plan is invoiced after a conversation) — and the header and footer "Pricing" entries are hidden there rather than promising a price list the page cannot show (#2474 assumed the page would be dressed; one line restores them when prices are published). The public project showcase `/projects` was `{ isPublic: true }` with no scope at all — one deployment listed every tenant's public projects, owner names included — and is now scoped to the request vertical, with the default vertical matching "not another vertical" so legacy projects with no `orgId` stay visible; MARKETING answers 404 because it does not carry the `projects` capability at all (#2473/#2499), rather than an empty grid promising a showcase this product does not have. Anonymous access is unchanged. Also: the `pricing` namespace joined `SERVER_ONLY_NAMESPACES` (its only reader is the server page), and the root layout now applies the vertical overlay BEFORE stripping those namespaces — stripping first merged them straight back in, so a marketing host was shipping the whole marketing `landing` overlay to the browser. Two follow-ups from review: the `messaging` card is core CRM so the capability filter cannot reach it, and its description was the last sentence on the marketing `/features` stating the internship relation model ("per-mentorship threads") — it now reads "a thread per deal" there through the vertical overlay, in all three languages; and the public live-demo card is tagged `mentorship`, because one demo instance exists and it runs the internship product (#2501 already hides its footer link on a marketing host). The root 404 page is vertical-aware too: it offered a Register button that host refuses, the mentor application, the partner-company pitch and a link straight back to `/projects` — the URL that had just answered 404.

## [0.210.0-beta] - 2026-09-23

_Shipped 2026-09-23 20:03 UTC · commit [ba5a668](https://github.com/21072026/Internship/commit/ba5a6689e184a9b6239dc57e0e5c5b6f1cb6f914)_

- **Lead attribution — "which source actually brings customers?" — now works for a marketing tenant, and counts against the tenant's own finished stage** (#2421, story #2393, epic #2348). No `Campaign` model was added: the existing `Source` row (plus the merged referrer pointer in `src/lib/referrer.ts`) carries marketing attribution, and the report's explicit `unsourced` bucket is the "no campaign" number the campaign report was wanted for. New pure, unit-tested `src/lib/leadAttribution.ts` owns two rules the two attribution routes used to restate inline: who may be counted toward a source — the funnel's lead side, and never a SOURCE login, whose `sourceId` records which source that account *speaks for* rather than who referred it — and the per-source intake/conversion arithmetic. `GET /api/admin/sources` also loses a wrong denominator: it divided a lead-filtered numerator by an unfiltered count of every account pointing at the source, so a source with its own partner login (the normal case on that screen — a SOURCE account's `sourceId` records which source it *speaks for*) reported half its real conversion. Both halves of every ratio now spread the same `attributedLeadWhere()` fragment. `GET /api/admin/sources` loses its last hardcoded `HIRED_660`/`EMPLOYED_700` literal (`outcomeStageKeys()`, #1882 — it reported 0% from every source for any org on its own stage catalogue, which is every MARKETING tenant, whose preset ends at `DEAL_WON`); `scripts/stage-keys-baseline.json` shrinks by that file. Reachability: a marketing admin gets there from `/admin/analytics`, which carries no capability tag, rather than from the `sourcing`-gated `/admin/sources` — `sourcing` is the partner-institution intake module, not a report, and widening it would have handed a marketing tenant the whole module. The premium gate deliberately stays for every vertical: `premiumAnalytics` already resolves per tenant, so which reports a tenant has is a plan question, not a product-line one. Labels come from the MARKETING terminology overlay (EN/TR/DE), not from new strings. E2E covers both halves — no Sources nav entry, attribution table on the analytics page — plus the folded #2398 / T-8.1.3 acceptance: an account created with source X makes X's count 1.

## [0.209.0-beta] - 2026-09-23

_Shipped 2026-09-23 20:02 UTC · commit [40450d4](https://github.com/21072026/Internship/commit/40450d455bdac954c0717ec2cb8374b42743d514)_

- **"My candidates" quick filter and bulk owner assignment on the candidates grid** (#2438, #2439, story #2397). `GET /api/candidates` takes `?mine=1`, which resolves to "has a LIVE `MentorshipRelation` whose `mentorId` is the signed-in user" — the owner field this repo already has; no owner column was added to `Company`. It is merged into the same relation `some` as the stage/company/project filters, so it intersects with status, tags, search and source instead of resetting them, and the toggle is written back to the URL (`replaceState`) so a filtered view can be pasted to a colleague. `POST /api/admin/candidates/bulk` gains an `assignOwner` action + `ownerId` field on the existing zod schema — no new route, no new selection machinery, still ADMIN-only, still capped at 200 ids, still `logActivity()`. It deliberately does NOT `updateMany` on `mentorId`: each row goes through `transferMentorship()` (#2289, docs/mentor-transfer.md), so a pairing with no history is corrected in place while one carrying work is closed `ENDED_REASSIGNED` and chained to a successor that keeps the stage — `InteractionLog` has no author column, and a bulk re-point would credit one mentor's meetings to another. Eligibility is per row (a candidate with no live pairing, or already on that owner, is skipped) and the response reports what actually moved, which the grid shows as "N reassigned". Each mentor involved is notified ONCE for the whole batch (`mentorship.bulkAssigned` / `mentorship.bulkReassignedAway`, in-app only) instead of once per row, and an error mid-batch is caught so the count already achieved is still returned (`partial: true`).

## [0.208.4-beta] - 2026-09-23

_Shipped 2026-09-23 13:42 UTC · commit [f5caa30](https://github.com/21072026/Internship/commit/f5caa3053794f02ae37363277df1c528505b3e98)_

- **The stage-SLA row squeeze is now pinned by a test that cannot pass vacuously** (#2373). The row's two number inputs left the German stage label 62px wide — narrower than the audit's own readability floor, i.e. a truncation that stops carrying information. #2508 fixed the markup (`w-16 sm:w-24`, giving the label 126px back), but nothing stopped it coming back, and the reason this issue was wrongly closed once is that nothing could see it: `StageSlaEditor` returns `null` when the org resolves no pipeline stages, so a freshly seeded admin walks the settings sweep straight past the component — the CI database had stages and the local container did not. The new case in `e2e/mobile-layout-coverage.spec.ts` stubs `/api/admin/stage-sla` with the real German labels (including the longest) so the rows are guaranteed to render whatever the database holds, then asserts that no row spills and that every label clears `MIN_TEXT_WIDTH`. Verified in both directions: green as the markup stands, and reverting #2508's classes reproduces `"100 · Erstkontakt" is 62px wide` — the original report's number, to the pixel.

## [0.208.3-beta] - 2026-09-23

_Shipped 2026-09-23 13:29 UTC · commit [c87564a](https://github.com/21072026/Internship/commit/c87564a48930983cbfc1153d5194c8e3cb0323d1)_

- **The duplicate-DOM race behind nine red e2e specs, fixed at both ends** (#2479, #2312, #2316 — one defect, three reports). A route with a `loading.tsx` is wrapped in a Suspense boundary, and React Fizz streams the resolved segment into `<div hidden id="S:0">` parked at the end of `<body>`: a complete second copy of the page, testids and DOM ids included. React 19 does not splice it in immediately — `$RC` schedules the reveal up to ~300ms later — so when hydration renders the content in place first, the document genuinely holds two of everything for that window. Playwright's strict mode counts hidden elements, so the first assertion after `page.goto` throws `resolved to 2 elements` rather than polling, and because a strict-mode violation is raised on the first query and never retried, it failed on the retry too. Confirmed against a production build of this app: the served HTML of every affected route carries that container, and for `/account` it opens with the page's own `<h1>`. Two changes. **The window is removed where it cost nothing:** `src/app/account/loading.tsx` is deleted — the page is a `'use client'` component with no server data, so the skeleton bought a blink and created the boundary that split the document; after the change `/account`'s served HTML has no `S:` container at all and `timezone-select` appears exactly once, which closes the route behind three of the four reports. **And it is waited out everywhere else**, since a `loading.tsx` over a server page that really fetches is worth keeping: `gotoSettled()` now waits for the staging container to be spliced away, and the nine specs that navigated with a bare `page.goto` go through it. That is a wait, not a mask — the container is only ever transient, so a duplicate that persists still fails the test. Written up as the third variant of the "unscoped testid resolves to 2" pitfall in `CLAUDE.md`, after the `AdminNav` search box and the responsive dual lists.

## [0.208.2-beta] - 2026-09-23

_Shipped 2026-09-23 12:47 UTC · commit [62c589e](https://github.com/21072026/Internship/commit/62c589e8d06966ccf15cd263c24f7c53124744fa)_

- **`/admin/analytics` fits a 360px phone, and the audit that was supposed to catch it can see the page again** (#1485; #1465 verified fixed). The funnel KPI card's conversion rows are four fixed columns — a 176px label, a 96px rate, a 112px count and three 8px gaps, 408px in a 278px box — so the screen spilled ~90px at phone width, worst in German where the stage labels are longest. A phone now gets two rows per stage (label + rate above, bar + count below) and keeps every number rather than hiding the denominator; from `sm:` up it is the original single flex row, geometry unchanged (measured at 1280px: label 176 → bar 191 → rate 96 → count 112, in that order). The reason this sat open is worse than the overflow: `settle()` in the layout audit waited for the account-menu button and for zero `.animate-pulse` skeletons, but a screen that fetches one aggregate rather than a list renders a bare centred "Loading…" with no skeleton anywhere — so the poll saw zero immediately and the audit measured an **empty page**. `/admin/analytics` reported CLEAN at 360px while still spilling 89px, and every element the audit exists to check was behind that line. Those placeholders now carry `data-testid="page-loading"` (17 of them, every screen that shows one) and `settle()`/`settleStandalone()` wait for it alongside the skeletons; the walks that visit several routes are marked `test.slow()`, because they now do real waiting instead of measuring six empty pages in a few hundred milliseconds. Re-running the audit with the honest wait reproduced #1485's CI output line for line before the fix, and #1465's mentee-onboarding-wizard measured 328px box / 328px content — clean, as #2282 left it.

## [0.208.1-beta] - 2026-09-23

_Shipped 2026-09-23 11:53 UTC · commit [9f01234](https://github.com/21072026/Internship/commit/9f01234060debe1a5d39143196a67cf836702c19)_

- **A date range that ends "today" now includes today** (#1501, #1484). Every screen with a range picker sends `to` as `YYYY-MM-DD` — what an `<input type="date"` holds, and what `toISOString().slice(0, 10)` produces — and `new Date('2026-09-23')` is that day's **first** instant. Used as an `lte` bound it excluded everything that happened *during* the day the admin picked, so on the analytics screen's default "6m" preset the trends chart, the interaction and meeting counts and the RSVP rate all silently omitted the current day. It read as a rendering bug rather than a data one: with a database whose rows were all created today — a CI shard's — the trends chart was twelve bars of zero, which is how it came back after #1425 had already fixed the layout underneath it and closed that exact symptom. The rule is now one dependency-free, unit-tested module (`src/lib/dateRange.ts`): a date-only bound names a **day**, so `from` is its first instant and `to` its last (`23:59:59.999`, inclusive, because two of the callers compare in JS with `<=` and an exclusive bound would be a trap); a bound that carries a time is taken literally. Local rather than UTC, because the month bucketing these ranges fill is local, and a range that disagreed with its own buckets would be a subtler version of the same bug. All six call sites read it: admin analytics, the funnel, the aging report, mentor analytics, the calendar window and the invitations board — the last of which had found the bug independently and fixed it inline, which is precisely why the other five still had it. Pinned by `scripts/test/date-range.test.mjs` and by an e2e regression that compares the bounded call against the unbounded one rather than asserting "> 0": a threshold passes on any database with history in it, which is how this shipped twice.

## [0.208.0-beta] - 2026-09-22

_Shipped 2026-09-22 15:05 UTC · commit [eb08011](https://github.com/21072026/Internship/commit/eb080113b1f382a3f196334ddc14702cc96195c2)_

- **The marketing account spreadsheet can be imported, idempotently** (#2404, #2405, #2406, #2407 — story #2391, epic #2348). A written column contract ([`docs/marketing-import.md`](docs/marketing-import.md)) plus a sample file with its four edge cases (`scripts/fixtures/marketing-accounts-sample.csv`), and an importer that runs on the SHARED engine — `runImport({ parse, validate, resolve, apply })` and the shared delimited parser in `src/lib/importPreview.ts`, the same ones the roster feed uses. No second parser, no second dry-run engine, and no `if (dryRun)` branch in the planning path: a dry run is the same call with a writer that does not write, and `npm run import:marketing-accounts` is dry-run by default (`--apply` to write, `--owner=<email>` for the organisation and the default funnel owner, following `scripts/import-csv.mjs`'s flags). `Company` gains `vatId`, `country` (ISO-3166-1 alpha-2) with `@@unique([orgId, vatId])` — org-scoped, never global, because two tenants may each hold the same merchant — and `contactName`/`contactPhone` beside the existing `contactEmail`; all four nullable, so `db push` applies to populated tables. Matching is VAT id first, then `normalizeNameKey(name) + country` through the repo's existing normaliser, which transliterates İ/ı/ş/ğ/ü/ö/ç before lowercasing (a plain `toLowerCase()` makes `İstanbul Tekstil A.Ş.` and `ISTANBUL TEKSTIL A.S.` two accounts on every run), and it is batched — four queries per run, not four per row. Because both new columns arrive with this change, every account a tenant already holds has `vatId` and `country` NULL, so a blank country on **either** side falls back to the name alone when exactly one stored account bears it (and fills the country in as a gap, so the next run keys exactly); two same-named accounts in different countries and a row naming none is reported `SKIP` as ambiguous, never guessed. The in-file duplicate guard keys on the account a row **resolves to** rather than the identity it claims, so one merchant listed twice with the VAT filled in on only one line is one account. A bad row is reported as `{row, key, status:'ERROR', reason}` and never thrown; a second apply of the same file is all `UNCHANGED`. The data-model decision the rest of the backlog rests on is now written down in `docs/marketing-vertical/pipeline-record.md` and linked from that folder's README: account = `Company`, funnel record = `MentorshipRelation` (owner = `mentorId`, lead person = `menteeId`, account = `companyId`), lead person = a `MENTEE` `User` — no second pipeline carrier, and stage history stays `StatusChange` on the relation rather than a Company-level table. A row with a stage but no primary contact keeps its account and skips the stage with a warning, because a funnel record needs a person. The lead `User` a placed row mints is a record and not a login: password-less **and** on a generated stand-in address on `import.local` derived from the contact address and the org id, because the sentinel password alone is not the guarantee — `/api/auth/forgot` mails a reset link to any existing user and `/api/auth/reset` consumes it, so the merchant's real address would have made every imported contact a claimable MENTEE account. The real address stays on `Company.contactEmail`. The new `vatId`/`contactName`/`contactPhone` columns are stripped for non-ADMIN readers of `GET /api/companies` and `GET /api/companies/[id]` (`src/lib/companyVisibility.ts`) so this does not widen the broad company read tracked as #2431. `importErrorMessage()` in the shared engine now reports the first NON-empty line and the error code: Prisma formats every P2002/P2000/P2003 starting with a blank line, so every database-raised import failure was previously reported as ERROR with an empty reason — the roster feed inherits the fix. Unit-tested dependency-free with an in-memory writer (`npm run test:marketing-import`, 35 cases, `src/lib/marketingImport.ts` floored at 95%).

## [0.207.0-beta] - 2026-09-22

_Shipped 2026-09-22 14:50 UTC · commit [c9802f9](https://github.com/21072026/Internship/commit/c9802f9cc9224c20468801ccab4a2ba820cc55fd)_

- **Trial stages in the marketing funnel, and the rule that decides who gets warned before one runs out** (#2413, #2412, #2414, story #2392, epic #2348). The `marketing_funnel` preset gains `TRIAL_ACTIVE` and `TRIAL_EXPIRED` between `LEAD_QUALIFIED` and `DEAL_PROPOSAL`; both are on-path (an expired trial is a record waiting for a decision, not a drop-out, so the later auto-advance needs no drop-off reason) and only `TRIAL_EXPIRED` carries an SLA — a trial already has a contractual clock and two clocks on one stage nag twice. `MentorshipRelation` gains nullable `trialStartedAt`/`trialEndsAt` (no new table, and deliberately not `stageDeadline`, which `stageSla.ts` rewrites on every stage change). New dependency-free `src/lib/trialReminderRule.ts` selects the due (record, threshold) pairs by CALENDAR-DAY difference in UTC, on an exact match: a trial ending seven days out is caught whatever hour the tick runs, the subject's number is always true, and an elapsed trial can never match again — which is what makes old claim rows prunable. 12 unit tests in `scripts/test/trial-reminder-rule.test.mjs` with fixed dates and an injected clock, floored at 95% line coverage. `src/lib/trialReminders.ts` adds the Prisma half (`findDueTrialReminders`), resolving the trial stage through the tenant's own `PipelineStage` rows so an org without one is skipped rather than swept. New `TrialReminder` claim table (`@@unique([relationId, threshold])`, cascade from the relation) registered in `TENANT_MODELS` and given a 365-day retention entry; it claims BEFORE sending, the house convention, because the thresholds are a ladder — a reminder lost to an SMTP hiccup is followed by the next one, whereas a duplicate is the mail that gets the sender filtered. A `trials` copy block lands in all three dictionaries, three subjects over one shared body, reusing the existing `stage-deadline` category with no new preference key. The DB round trip — candidate found, claim row written, second run selects nothing — is covered by `e2e/trial-reminders-query.spec.ts`; the marketing funnel's own description is updated to nine stages in all three languages, and the house stage-count bound stays at eight for every shape that does NOT carry the trial pair. The job that sends them is #2415.

## [0.206.2-beta] - 2026-09-22

_Shipped 2026-09-22 14:50 UTC · commit [a787643](https://github.com/21072026/Internship/commit/a787643ae418bd4dbcdf4bb7b7ce5fa25e978f70)_

- **Write-path capability gate on the placements module, and a check that keeps it complete** (#2364, epic #2348). #2351 dropped `/admin/offers`, `/admin/requisitions`, `/admin/interview-requests` and `/interviews` from the sidebar of a vertical without `placements`, and #2352 gated the mentorship modules — but the placement routes themselves were still reachable by a direct POST. All ten mutating handlers (`/api/offers` POST + `[id]` PATCH, `/api/requisitions` POST + `[id]` PATCH, `/api/interview-requests` POST + `[id]` PATCH, `/api/interview-panels` POST + `[id]` PATCH + `[id]/close` + `[id]/reopen`) now call `requireCapability(session.user.orgId, 'placements')` immediately after their session check and before any role check, validation or write, so a refused tenant gets `403 { code: 'capability_unavailable', capability }` rather than a 400 that leaks the shape of a module it does not have. Panel SCORING stays under `evaluations` (#2352) because it writes `Evaluation` rows; convening a panel is the placement. Two writers are deliberately left open and documented: `expireOffers()` (the cron expiry sweep — sessionless by design, and an already-sent offer should still stop being open) and the `InterviewRequest` re-pointing inside the duplicate-user merge (an identity operation that creates no placement). The leaked-writer class had been caught twice by human review and never by a check, so `npm run check:capability-writers` now walks back from every write to a gated model to the handler that reaches it and fails CI when there is no gate in between — including a second handler added to an already-gated file; the guard has its own unit tests for both the too-narrow direction (a `GET` declared under a gated handler no longer inherits its gate) and the too-broad one (a gate written as `requireCapability(resolveOrgId(session), 'placements')` is recognised as a gate). INTERNSHIP carries `placements`, so this is a strict no-op for today's product, asserted by the INTERNSHIP half of `e2e/vertical-write-gate.spec.ts`.

## [0.206.1-beta] - 2026-09-22

_Shipped 2026-09-22 14:50 UTC · commit [69365c1](https://github.com/21072026/Internship/commit/69365c16f89d3a493ce16a99c13e9037c627c8e0)_

- **A marketing tenant's company list and stage board stop speaking mentorship** (#2426, #2427, story #2394). `src/i18n/verticalOverlays.ts` gains MARKETING overrides in EN/TR/DE for every string those two *screens* render — the namespaces `companiesPage`, `board` and `adminBoard`, plus the surfaces the pages reach that live elsewhere: the two day-one empty states (`emptyStates.companies.adminBody`, `emptyStates.board.adminBody`), the company create/edit dialog (`companyForm.quota`/`needs`/`noNeeds` — "Internship Quota", "Internship Needs", "…to add internship positions"), the premium-features dialog behind the Sparkles button on every company card (`entitlements.subtitle`, "Mentor and mentee features are always free") and the person hover card every board owner chip opens (`personCard.roleMentor`/`roleMentee`, `candidates.mentor` — the same two words `dashboard.mentors`/`mentees` already rename). On the page itself: the subtitle ("…their internship needs"), the company-login hint, the two counter badges (`mentorships`/`positions` → deals/needs), `board.emptyStage`, the board subtitle and search placeholder ("Find a mentee or mentor…"), the WIP-saturation note and all three group headings. `adminBoard.groups.custom` is included because MARKETING_FUNNEL's stage keys are its own (`LEAD_*`/`DEAL_*`), so `groupResolvedStages()` files every column under `custom` — "Custom stages" was the one heading a provisioned marketing tenant actually saw; `pre`/`internship` are still overridden for an org switched to MARKETING while sitting on the canonical stage keys. No new dictionary key, no new table and no second translation layer for stage names: those come from the template's own `labels` through `stageLabel()`. `e2e/vertical-terminology.spec.ts` now visits both screens as a MARKETING admin (with the vertical's stage preset seeded, as `provisionStagePreset()` writes it), *opens* both dialogs and the hover card, and asserts the rendered text of the content region matches no mentorship word — with mirror assertions that an INTERNSHIP admin still reads the original strings in the same places. `e2e/vertical-overlays.unit.spec.ts` adds the dictionary-level half: no key of those namespaces reads mentee/mentor/internship (or staj/Praktikum) under the MARKETING overlay in any of the three locales, so a key added to one of them later is checked the day it lands.

## [0.206.0-beta] - 2026-09-22

_Shipped 2026-09-22 14:50 UTC · commit [5698e2d](https://github.com/21072026/Internship/commit/5698e2d00104bc26593330fcffe6c6411115bc35)_

- **Analytics page gated by the tenant's vertical** (#2423, story #2393). `/admin/analytics` is a core-CRM screen a MARKETING admin already reaches, but half of its cards described modules a marketing tenant does not have: mentor workload & outcomes, mentor capacity, meeting RSVP acceptance, the self-service signup funnel, intern projects and match quality. `GET /api/admin/analytics` now resolves the org's module set through the same `shellCapabilities()` path the sidebar and the write gate use and returns it as `capabilities`; the page (and the printable full report) hides the `mentorship`-only cards and the `projects` card for a vertical that lacks the module, and `GET /api/admin/analytics/funnel` no longer runs the mentor-capacity query for such a tenant (empty `capacity`). INTERNSHIP carries every capability, so its payload and screen are unchanged — `e2e/vertical-analytics-gate.spec.ts` asserts both verticals. The headline conversion tile — computed from the resolved finished stage since #2385 (closing #1882) — is now asserted on screen too, labelled from that stage: `Custom F rate` for a renamed pipeline (`e2e/custom-pipeline-analytics.spec.ts`) and `Won rate` for the marketing funnel, which closes #2419.

## [0.205.5-beta] - 2026-09-22

_Shipped 2026-09-22 14:50 UTC · commit [6f76b8b](https://github.com/21072026/Internship/commit/6f76b8bc58d738a0b63a31a5c0605b1dd4e0f785)_

- **Company records are role-scoped** (#2396: #2429, #2430, #2431, #2432). `GET /api/companies` and `GET /api/companies/[id]` stopped at `if (!session)`, so every signed-in role — MENTEE and SOURCE included — read every company in the tenant, `contactEmail` and address included. `authzScope.ts` gains a `company` resource (ADMIN all, COMPANY its own row, MENTOR the companies of relations it is named in; MENTEE/SOURCE have no builder → 403 + `authz.scope_denied`), the two routes and the requisitions company picker read it, the detail route's nested relations and the list route's `_count.mentorships` follow the `relation` scope, and `docs/role-access-matrix.md` gets the MARKETING section that decides all five frozen roles. `GET /api/mentorship/[id]` also stops handing the relation's mentor and mentee the whole `Company` row (`contactEmail`, `address`, `description`, `quota`) and selects `{ id, name, industry }` like every other indirect reader. Locked by two new rows in `e2e/fixtures/authz-matrix.ts`, a named case per scoped role and a per-role unit test.

## [0.205.4-beta] - 2026-09-22

_Shipped 2026-09-22 10:27 UTC · commit [ab2bdfc](https://github.com/21072026/Internship/commit/ab2bdfc0942d846b56acd5f590879e4f7ce36c3b)_

- **An instant call from a project's group chat is gated on the projects module** (#2504, split out of #2503). `POST /api/meetings/instant` ran `requireCapability(orgId, 'projects')` only when the request body carried a `projectId`, and a call started from a project's group room carries `conversationId` instead — so the module was closed for *creating* that room (`POST /api/conversations` has always been gated) and open for holding a call in it. The room's own project is knowable only once the meeting context resolves, so `resolveMeetingContext` now reports it as `owningProjectId`, deliberately separate from the context's `projectId`: a meeting still hangs off exactly ONE context (#1051), so the row is written with `projectId: null` as before and only the gate reads the owning project. Null for a RELATION and for a DIRECT chat, both of which are core mentorship rather than the projects module. The gate itself sits after the context resolves and before the room link is resolved or anything is written, and skips the work when the body-level gate has already covered the same project. Covered in `e2e/vertical-write-gate.spec.ts` by a pair the empty-POST table cannot express: a MARKETING org is refused with `capability_unavailable` and writes neither a `Meeting` nor the chat line, and an INTERNSHIP org makes the identical call successfully.

## [0.205.3-beta] - 2026-09-22

_Shipped 2026-09-22 10:16 UTC · commit [9083b49](https://github.com/21072026/Internship/commit/9083b4929889cc472f800410a1942860a7eee962)_

- **A call can no longer be started in a group room whose project was deleted** (#2503, following #2487 for `MeetingSeries`). `Conversation.projectId` is nullable with `onDelete: SetNull`, so a project's GROUP room outlives the project as an orphan while every `ConversationParticipant` row survives — the project-delete path never prunes them. The messaging layer is fail-closed on that orphan for everyone (`getConversationIfAllowed` and `canPostToConversation` both refuse a GROUP with no `projectId`, admins included), but `resolveMeetingContext`'s CONVERSATION branch queried `conversationParticipant` directly and never loaded the `Conversation` row, so it never saw `type` or `projectId`: reading the room and posting to it answered 403 while `POST /api/meetings/instant` answered 201, created the `Meeting`, wrote the "📹 … started a meeting" line into the very thread nobody may post to (#1055), and mailed an invite to every historical participant. Starting a call in a chat *writes* into that chat, so the rule for starting one is now the rule for writing one: the branch authorizes through `getConversationIfAllowed` + `canPostToConversation` rather than its own participant lookup. `canPostToConversation` already requires participation, so the replaced check is subsumed, not dropped — a non-participant is still refused, admin or otherwise. Two specs in `e2e/instant-meeting-team.spec.ts` built **projectless** GROUP rooms, a state the app's own invariant does not allow (its free-room test says so in a comment); both are now project-backed, so each fails for the reason it names, and a regression case covers the orphan end to end. The capability-gate half of the same endpoint is tracked separately in #2504.

## [0.205.2-beta] - 2026-09-22

_Shipped 2026-09-22 09:59 UTC · commit [7529409](https://github.com/21072026/Internship/commit/7529409edb1ba9f638276e4ea9b1d7254f99930d)_

- **An empty `MARKETING_HOSTS` no longer switches the marketing domain off** (#2428, epic #2348). `infra/deploy-prod.sh` threads the variable into the container as `-e MARKETING_HOSTS="${MARKETING_HOSTS:-}"`, so prod — which by design configures nothing — ran with an empty string, `?? 'marketing.ersah.in'` took that as a configured empty list, and https://marketing.ersah.in served the internship landing. `marketingHosts()` in `src/lib/servedHosts.ts` (shared since #2488 with the redirect allowlist, so the empty value also made the marketing domain a non-served host) now treats unset, empty and whitespace-only alike: the default applies. Unit-tested. `infra/README.md` documents the marketing host end to end (DNS, `*.ersah.in` cert, Caddy site file, `MARKETING_HOSTS` semantics, how to verify).

## [0.205.1-beta] - 2026-09-22

_Shipped 2026-09-22 05:48 UTC · commit [4c2eb8b](https://github.com/21072026/Internship/commit/4c2eb8bfa2938e0b3cf8b38bebbf9fd001544fb3)_

- **Stage-SLA row labels were unreadably narrow on a phone in German** (e2e-full red on `mobile-layout-coverage.spec.ts`, `/admin/settings`). Each row in `StageSlaEditor` is `<label> <days input> <wip-limit input>`, with both number inputs pinned to `w-24` (96px) regardless of viewport; at 360px the two fixed-width inputs plus gaps left the flex-1 label only ~62px to truncate into — well under the layout audit's 110px readability floor — for every German pipeline-stage label (`"220 · Warte auf Freigabe"`, `"490 · Praktikum abgeschlossen"`, …). The two inputs (and their `Days`/`Board limit` column headers) now use `w-16 sm:w-24`, freeing enough width for the label to clear the floor on a phone while staying byte-identical at `sm:` and above; the label also gains `min-w-0` so `flex-1 truncate` can shrink past its content's intrinsic width rather than relying on there being enough slack to avoid it. Desktop is unaffected.

## [0.205.0-beta] - 2026-09-21

_Shipped 2026-09-21 22:32 UTC · commit [e60b9c5](https://github.com/21072026/Internship/commit/e60b9c5014f4dd1b3937f4c938459f2e4e417911)_

- **The marketing vertical wears SaleVali's own colours and mark** (#2492, epic #2348). A new `magenta` accent joins the picker for everyone and is the MARKETING vertical's default (`defaultAccentFor` / `resolveAccent(preference, vertical)` in `src/lib/accent.ts`): the scale is anchored on salevali.de's own palette (50/100 = brand tints, 500 = brand primary `#cc33e5` for fills, 700 = brand dark `#9b1dbd`) with 600 = `#b929cf`, chosen so white text on primary buttons is 4.9:1 and link text 4.7:1 on the page body — the brand primary itself is 4.1:1 and would have regressed WCAG AA. The graduation cap gives way to the SaleVali "V" (`SaleValiMark`, cropped from the brand SVG with its gradients intact and collision-safe ids) through one `BrandMark`/`BrandTile` pair used by the header, footer, sidebar wordmark, release-notes link and the five auth pages, where the mark sits on the brand's deep-purple tile (10.9:1) rather than on magenta, where its orange tail vanishes. The favicon, Apple touch icon, browser tint (`generateViewport`) and the web-app manifest (name, icons, `theme_color`) follow the host too — `app/icon.svg` moved to `public/` so the `<link rel="icon">` can branch, and the manifest reads the host because a manifest fetch carries no cookies. INTERNSHIP is unchanged: blue accent, cap, the same manifest and icons. The profile validator (`PUT /api/profile`) now reads the colour list from `ACCENT_COLORS` instead of carrying its own six-entry copy — the copy rejected Magenta with a 400 while the picker offered it, and `e2e/accent-color.spec.ts` now picks the newest colour so the next addition trips the same test. Review follow-ups: the marketing `<link rel="icon">` list carries a PNG fallback (Safari loads no SVG favicon and fell through to the implicit `/favicon.ico`, the cap); `/messages` gets its own `generateViewport()` reading the shared `themeColorFor(vertical)` (its static `#1D4ED8` painted a SaleVali install's status bar blue on the very screen its manifest shortcut opens — the manifest reads the same helper); and `scripts/generate-pwa-images.ts` — which still read `src/app/icon.svg` and crashed — now renders every raster (manifest icons, a *full-bleed* maskable icon, apple-touch icon, shortcut tiles, iOS splash screens) for **both** marks from `public/icon.svg` / `public/icon-salevali.svg`, so the marketing manifest's shortcuts sit on the purple tile and `appleSplashLinks('salevali')` has images to point at. The internship rasters are regenerated by the same code path; the one visible change there is `apple-touch-icon.png`, now full-bleed (iOS painted its transparent corners black).

## [0.204.2-beta] - 2026-09-21

_Shipped 2026-09-21 19:58 UTC · commit [eb79038](https://github.com/21072026/Internship/commit/eb79038489f0d158217d0b1b359faa855a06d5b9)_

- **A visitor stays on the host they are on — the marketing site no longer bounces people to the internship site** (#2488, epic #2348). One container serves several public hosts, but NextAuth resolved every `callbackUrl` against `NEXTAUTH_URL` (the internship host) and never saw the request, so signing out on `marketing.ersah.in` — or being sent back to sign-in after a password change, a sign-out-everywhere or an account deletion — landed on `interncrm.com`; the SSO and Google-Calendar failure redirects and the `/release-notes` feed link were built from the same env value. New `src/lib/servedHosts.ts` (zero imports, unit-tested) derives the set of hosts this deployment serves from config that already exists — the hostname of `NEXTAUTH_URL`/`NEXT_PUBLIC_APP_URL` plus `MARKETING_HOSTS` — with exact-match only: no wildcard, no suffix rule, no new env var (a topic env is covered because its own host is its `NEXTAUTH_URL`). `authOptions.callbacks.redirect` now keeps an absolute `callbackUrl` iff its host is in that set (a relative one still resolves against `baseUrl`, NextAuth-default parity; `interncrm.com.evil.example`, `https://interncrm.com@evil.example`, comma lists, received ports and non-https schemes all fall back to `baseUrl`); `signOutEverywhere` and the SSO completion send an absolute same-origin `callbackUrl` (`absoluteHere` in `safeRedirect.ts`); the SSO login and Google-connect routes and `publicOrigin()` derive their origin from the validated request host (`requestOrigin`), never downgrading the protocol and never trusting a received port. Byte-identical on the internship host. Still on the other host by design and tracked separately: the SAML ACS and the Google OAuth redirect_uri (registered at the IdP/Google) and e-mail links (`NEXT_PUBLIC_APP_URL`).

## [0.204.1-beta] - 2026-09-21

_Shipped 2026-09-21 19:35 UTC · commit [51d6592](https://github.com/21072026/Internship/commit/51d65927e943b31a0d07f978db60a46ad57c4cf5)_

- **Orphaned meeting series are no longer anyone's to cancel** (#2487). `MeetingSeries.projectId` is nullable with `onDelete: SetNull`, so a series whose project was deleted survives as an orphan — and `DELETE /api/meeting-series` ran its project-access check and the `'projects'` capability gate only inside `if (current.projectId)`, leaving any mentor in the organisation free to deactivate (and purge the generated meetings of) any such row; `PUT` checked only the *target* project, so the same row could be adopted into a project of one's own and edited from there. A new `ensureSeriesAccess()` decides for every existing row: with a project, the project (unchanged); without one, its author or an admin — the line `project-tasks` already draws for a personal to-do. `DELETE` and `PUT` both check the current row through it (a move additionally checks the target project), and `DELETE`'s capability gate now sits at the top of the handler, unconditional, like `POST`/`PUT`. `e2e/meeting-series.spec.ts` gains the orphan case. Read paths were never affected: `GET` requires a `projectId`, the reminder cron skips project-less series, the calendar expands through projects.

## [0.204.0-beta] - 2026-09-21

_Shipped 2026-09-21 10:28 UTC · commit [c81382a](https://github.com/21072026/Internship/commit/c81382a55d7f9411bba4c3016c7287815b41da8d)_

- **Interview decline reasons** (#1416). Declining an interview request now requires a coded reason, preserves the company's original request note, and shows the localized reason to company users on the request card and in their notification.

## [0.203.2-beta] - 2026-09-15

_Shipped 2026-09-15 22:46 UTC · commit [1da2aa6](https://github.com/21072026/Internship/commit/1da2aa694ee5125ea8af7b40efbcc944d464834d)_

- **A marketing visitor no longer leaks into the internship product through the chrome, the auth pages or the CTA** (#2356, epic #2348 — adversarial-review follow-up to #2474). The public header's wordmark was still the hard-coded string "InternshipCRM" and its aria-label said so; the footer tagline described mentors and mentees; the register page said "Sign up as a mentee — no invitation needed" and offered the mentor application; the sign-in page said "Sign in to your InternshipCRM account"; the landing's only conversion button, "Get started", pointed at `/auth/register`, where a token-less sign-up mints a **MENTEE in the internship default org**; and three demo links led to the internship demo. Now: a `useVertical()` client seam (`src/lib/verticalClient.tsx`, fed by the root layout through `Providers`) lets client components read the request's vertical, so the header wordmark is the vertical's product name; a marketing host shows no Register button and no demo links, its CTA is "Sign in", its register page opens the invitation field by default and hides the mentor link, and **`POST /api/register` refuses a token-less sign-up on a marketing host with 403 `invitation_required`** — marketing accounts are created by invitation. The MARKETING overlay now also covers `publicNav.homeLink/tagline` and `auth.signinSubtitle/registerSubtitle/tokenHint`, and four landing claims the review found untrue for this vertical were corrected (lead-source tracking needs the `sourcing` capability MARKETING lacks; merge is for people, not accounts; stage editing is a premium feature; every plan has a seat limit) plus the funnel box label now matches the preset ("Proposal sent"). One deliberate INTERNSHIP-visible detail: the public header's wordmark now reads the product name "Internship CRM" — what the sidebar, the footer (#2458) and the tab title already show — instead of the one-word literal "InternshipCRM" it alone still carried; everything else on the internship host is unchanged.

## [0.203.1-beta] - 2026-09-15

_Shipped 2026-09-15 22:39 UTC · commit [e553964](https://github.com/21072026/Internship/commit/e553964def34a3fe96828d8a2e1a076b0cf814eb)_

- **Every project-owned write is now behind the `projects` capability** (#2502, epic #2348 — adversarial-review follow-up to #2473). #2473 gated the six obvious project routes but the review found six more paths that create or change project-owned rows without a gate: `PATCH`/`DELETE /api/project-tasks/[taskId]` (project-bound tasks only — personal to-dos stay open), the PROJECT_TASK branch of note→task conversion (`/api/notes/[id]/convert`, whose own comment promised a gate that was never added), `POST /api/invite` with a `projectId` (acceptance upserts a `ProjectMember`), a project-bound contributor-terms acceptance, `POST /api/conversations` with a `projectId` (the project's group conversation), and the project meeting writers (`/api/meeting-series` POST/PUT/DELETE, `/api/meetings/instant` with a `projectId`). All now call `requireCapability(orgId, 'projects')` and answer `403 capability_unavailable` for a tenant without the module; the gates that depend on a `projectId` in the body are conditional so the non-project use of the same route is untouched. `e2e/vertical-write-gate.spec.ts`'s GATED table gains the six project routes whose gate an empty POST reaches, so a regression is visible to CI. INTERNSHIP carries every capability — no behaviour change there.

## [0.203.0-beta] - 2026-09-15

_Shipped 2026-09-15 19:57 UTC · commit [70f6016](https://github.com/21072026/Internship/commit/70f60163e091e71bf20159d1dae9c17b47825a5a)_

- **The marketing host now has its own landing page** (#2356, epic #2348). Below the hero, the marketing landing still rendered the internship argument in full — the mentee/mentor/company loop, the role picker, three audience sections written for students, mentors and hiring companies, the free-core band, the how-it-works steps, the five-roles grid, mentor/mentee success stories, a per-role FAQ, and "Apply as a mentor" / "Talk to us as a company" CTA buttons. Those sections describe a mentoring loop and are nonsense to a sales team, so for a MARKETING host they are no longer rendered; the page is now hero → feature cards → funnel diagram → "and a lot more" → transparency → one CTA. The sections that remain are re-worded through the MARKETING overlay (leads, accounts, deals; a New lead → Contacted → Proposal → Won funnel; a marketing CTA), and the featured feature cards are filtered by the vertical's capabilities so a marketing visitor is not sold "Mentor self-service" or intern projects (`Feature.capability`, `src/lib/features.ts`). The internship landing is byte-identical.

## [0.202.5-beta] - 2026-09-15

_Shipped 2026-09-15 19:45 UTC · commit [5d1ae5f](https://github.com/21072026/Internship/commit/5d1ae5f5561131ef842440a655ea7ddff29d2c84)_

- **A marketing tenant no longer sees — or can write through — the internship-only modules, and its sidebar is the same on every page** (#2356, epic #2348). Three fixes. (1) `projects` left the MARKETING capability set: intern team/task projects are not a marketing concept, so `/admin/projects`, `/mentor/projects`, `/portal/projects` and the contributor-terms page drop out of a marketing tenant's navigation, and every project write API (`POST /api/projects`, project `PUT`/`DELETE`, members, join requests, task templates, tasks) now refuses with `capability_unavailable` for a tenant without it — hidden nav alone would still have let a hand-typed URL create one. (2) Goal templates, cohorts and success stories are gated on `mentorship`, and the partner-company enquiry inbox on `placements`, so they too leave the marketing sidebar. (3) `RoleShell` — the shell behind `/todos`, `/interviews`, `/messages`, `/newsletters`, `/mentors` — never resolved the tenant's capabilities, so `AdminNav` fell back to 'not passed = every link' and a marketing admin saw the full internship menu on exactly those pages while `/admin/*` showed the filtered one; the shell now resolves `shellCapabilities(orgId)` and hands it to AdminNav, MentorNav, PortalNav and the command palette. INTERNSHIP carries every capability, so nothing changes for it.

## [0.202.4-beta] - 2026-09-15

_Shipped 2026-09-15 09:15 UTC · commit [802193b](https://github.com/21072026/Internship/commit/802193b56e11d53fc9fe9fea0e7a47b996400458)_

- **Aşama saati ve süre raporu artık eski no-op `StatusChange` satırlarını hareket saymıyor** (#2264). #934 yeni no-op satır (`fromStatus === toStatus`) yazılmasını durdurdu ve eski satırların *kalmasına* da bilerek karar verdi — satır bir admin'in kendi gerekçe kodunu taşıyabiliyor ve tablo bir denetim kaydı. Ama sayılar listeden değil tablodan hesaplanıyordu, dolayısıyla arayüzde gizlenmiş olmaları onları hesaptan düşürmüyordu. En kötüsü saatti: `stageEnteredAt()` **en yeni** satırı alır, yani tek bir eski no-op satır "aşamada geçen gün"ü sıfırlıyor, aylardır kımıldamamış bir ilişki panoda, mentor listesinde ve gecikme raporunda taze görünüyordu. Süre raporunda ise satır "X'ten çıktı, X'e girdi" diye okunup devam eden ziyareti erken bitiriyor, sıfıra yakın ikinci bir ziyaret kaydediyor ve o aşamanın ortalaması ile medyanını aşağı çekiyordu — raporun üretmek için var olduğu tek sayıyı. Düzeltme tamamen okuyucu tarafında, **hiçbir satır silinmiyor**: `stageChange.ts` artık kuralın yazma kapısının yanında okuma `where`'ini de veriyor (`REAL_STAGE_MOVE`, Prisma alan referansıyla `fromStatus <> toStatus`) ve saati besleyen her sorgu onu kullanıyor — `orderBy createdAt desc, take: 1` çeken yerlerde şart, çünkü orada sonradan filtrelemek no-op'un sakladığı hareketi geri getiremez; `stageClock.ts` aynı kuralı tüm geçmişi elinde tutan çağrı yerleri (mentee detayı, gecikme raporu) için bellekte uyguluyor (`fromStatus`/`toStatus` opsiyonel: sorguda filtrelenmiş satır iki kolonu boşuna çekmiyor); `computeStageAging()` no-op satırı ziyaret saymıyor; mentor analitiğindeki "aşama hareketi" sayacı ile `avgDaysToHired`, ve mentee aktivite raporundaki aşama sayacı aynı filtreden geçiyor. Gecikme raporundaki ayrılma-nedeni sayacı da öyle: off-path bir aşamada duran eski bir no-op satır, kendi gerekçe koduyla birlikte yeni bir ayrılma gibi sayılıyordu. Kural birim testli (`scripts/test/stage-clock.test.mjs`, `stage-aging.test.mjs`'e iki vaka) ve `e2e/stage-clock.spec.ts`'te uçtan uca sabitlendi — düzeltme geri alındığında test kırmızıya dönüyor, doğrulandı.

## [0.202.3-beta] - 2026-09-15

_Shipped 2026-09-15 09:03 UTC · commit [25035b3](https://github.com/21072026/Internship/commit/25035b3b2f30d80dfd0dbf1ba0abdf3ffc11ecd2)_

- **e2e: a rate-limit flood no longer spends everybody else's bucket, and CI now says so before the scheduled run does** (#2159, #2158). `enforceRateLimit` keys on `bucket:ip` into a per-process store, and a full-suite shard runs its whole file list against one Next server (`workers: 1`, one `webServer`) — so every request that arrives without a usable client address counts into the same `<bucket>:unknown` counter, the suite's calls to an endpoint are cumulative, and the windows are 10-60 minutes, longer than a shard takes. `rate-limit.spec.ts`'s six POSTs to `/api/support` (limit 5) therefore left `support-attachments.spec.ts` and `support-chat.spec.ts` with nothing to spend and pinned shard 4/4 of `e2e-full` red on the same two tests every non-skipped run; `mentor-applications.spec.ts` was at exactly 5/5 on its own and handed the 429 to whatever ran next. New `e2e/helpers/rateLimit.ts` hands out synthetic `X-Real-IP` addresses — `floodIp()` a stable one for a test that is *measuring* the brake and must spend one counter repeatedly, `freshIp()` a new one per call for a spec that merely has to drive a rate-limited endpoint to reach the feature behind it. It goes through `x-real-ip` rather than `X-Forwarded-For` precisely because the webServer runs with `TRUSTED_PROXY_COUNT=0` so the #858 spoofing test can prove a rotating forwarded header buys nothing — that test keeps the address it did not choose, since sharing is its whole claim. `support-chat.spec.ts` alone posts seven support messages against a limit of five, so a per-spec budget would only have moved the cliff. Browser-driven sends are covered too (`extraHTTPHeaders` on the context, `page.setExtraHTTPHeaders` on the default fixture). The class is closed rather than the instances: `npm run check:e2e-rate-limits` (new, in `ci.yml`) reads every bucket's limit and window straight out of the route files and counts the unisolated e2e call sites that spend it — cumulatively for a window longer than five minutes, per spec file for a shorter one, skipping buckets keyed on a `subject:` rather than an IP — and fails the PR when a bucket is within reach. It found `register` sitting at 13/15 before anything was red; those thirteen call sites are isolated here too. No product code changes: the limiter is unchanged and still asserted.

## [0.202.2-beta] - 2026-09-15

_Shipped 2026-09-15 08:42 UTC · commit [aab9e84](https://github.com/21072026/Internship/commit/aab9e84e0d03960cc884f239295e1714dd3cfe03)_

- **The mentor dashboard's mentee-onboarding wizard stops pushing the page past a 360px phone** (#2282). Its cards sit in a grid, and a grid item defaults to `min-width: auto` — so the widest unbreakable row in a card (a long German mentee name beside a badge and two 44px controls) set the track width instead of the name truncating, and 328px of column carried 361px of content. `min-w-0` on the card lets the track win and the name truncate as it was always meant to. The step labels in the expanded state gain `min-w-0 break-words` for the same reason: they are full German sentences. Also `test.slow()` on the 320px/400%-zoom reflow test — it sweeps two viewports over several routes in two locales, about 78 seconds against a warm server where the project default is 60, and was failing as a timeout that reads like a layout finding and is not one.

## [0.202.1-beta] - 2026-09-15

_Shipped 2026-09-15 08:18 UTC · commit [e8c72e7](https://github.com/21072026/Internship/commit/e8c72e74091511600549a00f0a3db81b74cba187)_

- **`/admin/offers`: a search no longer drops the preset it was typed into** (#2161). Two races, one screen. `setParams` built the next URL from the render-time `useSearchParams()` snapshot it closed over, and `router.replace()` does not synchronously re-render — so a preset click followed by a search submit in the same tick merged `q` onto the **pre-preset** params, silently dropping the preset's `status` and listing offers from outside the view the admin was looking at. It now writes through a ref that is authoritative the instant a navigation is issued (and is re-synced from the URL for a back button or a link). Second, the search box was React state that other handlers reset imperatively — both `applyPreset` and the clear button called `setSearch('')` — so a keystroke arriving between the click and React flushing that reset was wiped and the form submitted an empty query. The field is now uncontrolled and keyed by the URL's own `q`: the DOM owns the text, and it is re-mounted only when the URL changes, which is the only time anything but the user should change it. The product question the issue left open is answered in the code: a preset names a **view**, the search box narrows **within** it, and choosing a preset clears the query — composing in that one direction, because replacing the preset from the search box would leave no way to search inside a preset at all. `e2e/admin-offers.spec.ts` is 5/5.

## [0.202.0-beta] - 2026-09-14

_Shipped 2026-09-14 08:42 UTC · commit [7c6c9fe](https://github.com/21072026/Internship/commit/7c6c9fefe5960131bb04c488bce0b75ad87aed4a)_

- **The marketing vertical now reads as its own product, not internship with a different logo** (#2356, epic #2348). A signed-in marketing tenant's admin dashboard, its first-run checklist and its people list still showed internship terminology — "Mentee", "Mentor", "Active Mentorships", "Overview of your internship management system", "Invite your first mentors" — and a signed-out marketing host's wordmark and browser tab still said "Internship CRM". The per-vertical terminology overlay was extended to cover the admin dashboard (leads / reps / deals) and the onboarding checklist, the people list is now "Leads" ("Müşteri Adayları") consistently rather than conflating leads with deals, and the signed-out wordmark and tab title now read the vertical's own product name (a `productName` per vertical in src/lib/verticals.ts). Internship is byte-identical — its overlay stays empty and its title unchanged.

## [0.201.11-beta] - 2026-09-14

_Shipped 2026-09-14 08:01 UTC · commit [8ce4463](https://github.com/21072026/Internship/commit/8ce4463cbc7546f39135fbab28dfcfeeaeabc41b)_

- **Server components now bind the tenant context before reading data** (#2356, epic #2349). The tenant-isolation middleware only scopes a Prisma query when a request has bound its org via `withTenantScope`/`runWithOrg`; API routes do this, but three server-rendered pages queried Prisma directly and so, under `MT_ENFORCE_ISOLATION`, read across tenants: the admin dashboard counted and listed **every** tenant's mentees/mentors/mentorships (surfaced on a preview marketing org whose dashboard showed the internship tenant's people while its own user list correctly showed one); the project detail page fetched a project by id with no org filter (a signed-in cross-tenant IDOR — an admin could open another tenant's private project, roster and join requests); and the re-engagement pool relied on a hand-rolled, not-fail-closed org filter. All three now bind the caller's org (`src/app/admin/page.tsx`, `src/app/projects/[id]/page.tsx`, `src/app/admin/re-engagement/page.tsx`), and a new isolation e2e pins the dashboard in both directions. No behaviour change while the flag is off; anonymous access to the public project showcase is unchanged. The cross-tenant public-project listing on `/projects` is tracked separately for host→org scoping.

## [0.201.10-beta] - 2026-09-14

_Shipped 2026-09-14 07:28 UTC · commit [0b04f9d](https://github.com/21072026/Internship/commit/0b04f9d37805949ce9361281fe4dd666ff7e9c98)_

- **`MARKETING_HOSTS` is now threaded to the container by the deploy scripts** (#2355, epic #2348). The host→vertical resolver (`src/lib/hostVertical.ts`) reads `MARKETING_HOSTS` to decide which public hosts show the marketing landing, but `infra/deploy-prod.sh` never passed it through, so only the built-in default (`marketing.ersah.in`) could ever work — a second marketing host (a preview host, a future prod host) had no way to take effect. The shared prod/preview deploy now passes `MARKETING_HOSTS` from the env file and captures it from a running container on redeploy, the same pattern `MT_ENFORCE_ISOLATION` uses. No behaviour change where the var is unset (the code default still resolves `marketing.ersah.in`); this unblocks pointing another host at the marketing landing on a given deployment.

## [0.201.9-beta] - 2026-09-14

_Shipped 2026-09-14 07:18 UTC · commit [36993a5](https://github.com/21072026/Internship/commit/36993a5c214e825c93764ad3fc9ca18c1ebd7f71)_

- **A timezone picked before `/account` finished loading is no longer silently reverted** (#2344). The control saves on change and the profile load sets its initial value, so a pick made while `GET /api/profile` was still in flight was overwritten by the response a moment later — the badge went on showing the browser's zone, and the user's choice was gone with no error. Locally that window is milliseconds; on a loaded machine it is wide enough to lose every time, which is how it surfaced as a scheduled-suite failure. The picker is now disabled until the profile has answered: before that it does not yet show the zone that is stored, so a pick would be made against the wrong current value in any case. `e2e/timezone-settings.spec.ts` gains a regression test that holds the profile response open and asserts the control is not operable until it lands — deterministic rather than a race the test hopes to win, and it fails without the fix.

## [0.201.8-beta] - 2026-09-14

_Shipped 2026-09-14 06:34 UTC · commit [60eb47c](https://github.com/21072026/Internship/commit/60eb47c7242291448f128728d1b374095ff786c4)_

- **`/admin/meetings` meets WCAG 2.2 SC 2.5.8 (Target Size) at phone width** (#2403). The mobile layout audit reported seven findings there: three browser-default `13×13px` checkboxes in the recipient list, and four 20px-tall `person-trigger` names. The spacing exception did not save them — each sits within 12px of a full-width date or filter control on the same screen. The checkboxes are now 24×24 (`h-6 w-6`, the criterion's minimum, since a bare `<input type="checkbox">` will always fail this rule beside another control), and `PersonHoverCard`'s trigger is `inline-flex min-h-6 items-center`: it is a `role="button"`, and wherever it renders as a flex item it is blockified and loses SC 2.5.8's *Inline* exception, at which point a 20px name next to a form field is an undersized target. Both fixes are in shared components, so every screen that uses them gains the larger target.

## [0.201.7-beta] - 2026-09-14

_Shipped 2026-09-14 06:27 UTC · commit [e884d40](https://github.com/21072026/Internship/commit/e884d4011c8b393be9443fdd77d0ae90ebdd44a5)_

- **Three screens stop spilling past a 360px phone in German** (#2310). `/admin/newsletters` overflowed by 48px at the page container and in eight nested boxes below it: the schedule control and its button could not wrap, and the tip rows' `flex-1` title input kept the intrinsic width of its German placeholder. `/projects` overflowed through `PublicFooter`'s link columns, where a long German label had no `break-words` — the same shape as #1497 on `/release-notes`. `/admin/api-explorer` overflowed inside Swagger UI's own server/scheme selector, whose `<select>` will not shrink; corrected in the component's scoped stylesheet (wrap the row, keep the boxes inside their column, let the control scroll) rather than at a source we do not own. `src/components/ui/Input.tsx` also gains `min-w-0` on its wrapper so a field can shrink when it is a flex item — without it a `datetime-local`, ~200px of browser chrome, drags its whole row past the viewport, which is what the nine `/admin/newsletters` findings cascaded from. Separately, the audit's own admin is now a **super-admin**: `/admin/organizations` is in the sidebar, a plain ADMIN 403s on it, and the route was therefore enumerated as covered while rendering nothing — an audited route that never renders reads as tested and is not.

## [0.201.6-beta] - 2026-09-14

_Shipped 2026-09-14 06:19 UTC · commit [8c30b3b](https://github.com/21072026/Internship/commit/8c30b3bdc1e40766acf6b4196aa6949f5f68895b)_

- **Every zod upper bound now fits the column behind it, and CI keeps it that way** (#2262, the remainder of #1433). A cap wider than its `VARCHAR(191)` column let long text pass validation and die in the `INSERT` with Prisma P2000 — which no route handles, so the user got a 500 for input the form had just accepted. Fixed at the bound rather than by widening columns: `Project.name` (200), `Project.repoUrl`/`demoUrl`/`boardUrl` (500), `Goal.title` (200), `MeetingRequest.topic` (300), `Document.title` (200), `Organization.brandName`/`supportEmail` (200) and `ssoIssuer` (500), `MentorApplication.linkedinUrl` (500), `CompanyInquiry.email` (200), plus `Organization.brandColor` and the profile fields `city`, `university`, `department`, `linkedinUrl`, `githubUrl`, `portfolioUrl`, which had **no** cap at all on either side. Each limit is a named constant in `src/lib/textLimits.ts` used by the server schema, the client schema and the input's `maxLength`. New `npm run check:text-limits` (in `ci.yml`) reads the registry that moved out of `e2e/text-limits-columns.unit.spec.ts` into `src/lib/textLimitsRegistry.ts` and enforces it in both directions for 12 models: every `String` column accounted for (a constant, or an exemption with a written reason), no stale row, the constant fits the width parsed from `prisma/schema.prisma`, and every guarding file references the constant instead of a literal. The invariant used to be checked only by the scheduled full suite, so a too-wide cap merged green and surfaced on the first long paste; the check reads nothing but files, so it now runs in the PR gate. Two entries of the issue's table were **not** violations and are left alone — `ProjectTaskTemplate.title` was already fixed by #1433, and the `description`/`goals` fields it lists are `@db.Text` rather than `VARCHAR(191)` (they gain constants anyway, so no literal is left in those routes).

## [0.201.5-beta] - 2026-09-14

_Shipped 2026-09-14 04:30 UTC · commit [a0e71b4](https://github.com/21072026/Internship/commit/a0e71b467d4636d84658ec20390479a8669f51e2)_

- Release compaction now moves `package-lock.json`'s version with `package.json`'s (the two fields npm keeps there), and `npm run check:release-fragments` fails when the two files disagree — `npm run fix:lockfile-version` repairs it in a 2-line diff. A clean checkout of `main` no longer goes dirty on `npm install` (#2243).

## [0.201.4-beta] - 2026-09-14

_Shipped 2026-09-14 04:28 UTC · commit [a9dd9c2](https://github.com/21072026/Internship/commit/a9dd9c229cdcf439f9ad773cf9f21a0398d492ee)_

- **Tenant isolation enforcement is now wired to the deploy path, and enforced in every topic environment** (#2356, epic #2349). `MT_ENFORCE_ISOLATION` was never passed to the container by any deploy script, so the Prisma tenant middleware stayed dormant everywhere; the deploy scripts now thread it through — `infra/server/topic-deploy.sh` forces it ON for every per-PR topic env (a fresh single-org DB, so enforcement is transparent, and this makes each PR the rollout's test bed), while `infra/deploy-prod.sh` (shared by prod and preview) passes it from the env file for a staged, per-environment rollout. A new isolation e2e (`e2e/isolation/user-pools.spec.ts`) pins the reported case in both directions: a tenant admin's candidate/user pool contains its own people and never the other tenant's, and a tenant still sees its own full pool under enforcement. No behaviour change for prod/preview until their env flag is set — this is the plumbing and the test bed; flipping preview then prod is the guarded next step.

## [0.201.3-beta] - 2026-09-14

_Shipped 2026-09-14 04:23 UTC · commit [28c80c9](https://github.com/21072026/Internship/commit/28c80c98963dbce6138f6f7c1acaed524d9a7f4d)_

- **The five paid reports count the tenant's own finished stage instead of a hardcoded `HIRED_660`** (#1882, story #1880). Cohort comparison, source conversion, the cross-program benchmark, the admin headline conversion and mentor analytics each carried their own `new Set(['HIRED_660','EMPLOYED_700'])`; a customer that renamed its pipeline (#747) holds neither key, so all five returned a confident zero — including the cohort route's `statusChanges.where.toStatus.in` filter, which left time-to-hire permanently `null`. One helper now answers the question for everybody: `outcomeStageKeys(orgId, locale)` / `outcomeStageKeysFrom(stages)` in `src/lib/pipelineStages.ts` returns `{ finished, finishedLabel, finishedLabelIsCustom, offPath, first }`. `finished` is anchored rather than "the last on-path stage" — the first on-path stage whose key is one of the canonical outcome keys, else the last on-path stage, and then everything ordered at or beyond it — because the built-in catalogue ends `HIRED_660 → EMPLOYED_700` and a naive "last stage" rule would silently stop counting `HIRED_660`. For an org with no `PipelineStage` rows the resolver falls back to the built-in catalogue and the sets come out byte-identical to the literals that were removed (`finished` = `['HIRED_660','EMPLOYED_700']`, `offPath` = `['INTERNSHIP_DROPPED_460','INTERNSHIP_FOUND_ELSEWHERE_800']`), so a single-tenant deployment's numbers do not move. The benchmark is the awkward one: it reads across ALL orgs behind a `MIN_RELATIONS = 5` k-anonymity floor, so it resolves a stage set per orgId from ONE batched `PipelineStage.findMany({ where: { orgId: { in: [...] } } })` — run through `runUnscoped` on purpose, because the tenant middleware's injection is an overwrite (`{ ...where, orgId }`) that would replace the `in` list with a single id and silently mis-count every other program. Only aggregate numbers still cross the boundary; only the viewer's own stage is ever named. Each payload now returns `finishedLabel` + `finishedLabelIsCustom`, and `/admin/analytics`, `CohortComparison`, `SourceConversion`, `ProgramBenchmark` and `/mentor/analytics` print the tenant's own stage name when the tenant renamed it and keep their existing EN/TR/DE wording when it did not — one new key trio, `analytics.conversionToStage`. Not a fork of #1504's placement definition: `src/lib/placement.ts` does not exist on `main`, and that issue is about which RELATIONS count while this is about which STAGES mean finished.

## [0.201.2-beta] - 2026-09-14

_Shipped 2026-09-14 04:23 UTC · commit [28c80c9](https://github.com/21072026/Internship/commit/28c80c98963dbce6138f6f7c1acaed524d9a7f4d)_

Fixed a ~1-in-256 CI failure: the stub IdP's hand-rolled DER encoder emitted a non-minimal INTEGER for certificate serials beginning with a zero byte, so `new X509Certificate()` threw at import and the Playwright webServer never started. Guarded by `npm run test:idp-mock-der`.

## [0.201.1-beta] - 2026-09-14

_Shipped 2026-09-14 04:22 UTC · commit [65db869](https://github.com/21072026/Internship/commit/65db8693ea77aecc409822f078ecedba8be0e8fb)_

- **A list filter can no longer replace the caller's data scope** (#2288). `GET /api/mentorship` composed its `where` by spreading the fail-closed scope from `scopeForRole()` into a `Record<string, unknown>` and then **assigning** `where.OR = […]` for `?search=`. The MENTOR and MENTEE relation scopes are themselves expressed as an `OR` (both sides of the relation, #1141), so a search term did not narrow the scope — it overwrote it, and one request from any signed-in mentor or mentee returned every relation in the database rather than their own. COMPANY and SOURCE were unaffected only because their scopes happen to use different keys, which is what makes this a property of the composition rather than of one route: the fix is `andScope(scope, …filters)` in `src/lib/authzScope.ts`, which puts the scope in its own `AND` conjunct where no later filter can reach it, and it is used by **every** `scopeForRole` caller — `/api/mentorship`, `/api/interactions` and `/api/projects` — so the next route to grow a filter inherits the property instead of re-implementing the convention. Empty and absent terms are dropped, so an unfiltered request keeps exactly the `where` it had before and the helper never emits `AND: []`. The routes now type their `where` as the real `Prisma.*WhereInput` instead of `Record<string, unknown>`, which is what let the key collision compile in the first place. Covered by a new `e2e/authz-matrix.spec.ts` case that signs in as one mentor and searches for the *other* seeded pair — the unsearched list was always correctly scoped, so the assertion has to sit on the search path.

## [0.201.0-beta] - 2026-09-14

_Shipped 2026-09-14 04:19 UTC · commit [f33c9ed](https://github.com/21072026/Internship/commit/f33c9edf26dda37ef40ff6936fe6a247a40794fa)_

- **2FA recovery codes: minted at enrolment, shown once, spent once** (#1542). Turning on two-factor authentication had no way back: a user who lost their phone was locked out until somebody edited MySQL by hand, which is the real reason `require2fa` was frightening to switch on. Enabling 2FA now returns **ten single-use recovery codes** in that one response and never again — the new `TwoFactorRecoveryCode` table stores only `HMAC-SHA-256(NEXTAUTH_SECRET, '2fa-recovery:' + normalised code)`, because a ~38-bit code behind a bare digest is enumerable offline from a database dump, and bcrypt is deliberately avoided on the verify side (ten cost-12 comparisons per attempt is a DoS lever on the sign-in path; a work factor protects low-entropy human input, which CSPRNG output is not). At sign-in, a submission that is not a live TOTP code and is *shaped* like a recovery code is tried as one, timing-safe against every unused row, and consumed by a conditional `updateMany` (`where: { id, usedAt: null }`) so two tabs cannot both spend it. Rate limiting is two buckets rather than one: a failed recovery attempt is charged to the existing `totp` bucket (5/15min, so the second door is not a way to brute-force *around* the authenticator's limit) **and** to its own new `recovery` lockout stage (5/60min, so it does not *inherit* that allowance and a slow-paced guesser still runs out). A successful recovery sign-in writes `2fa.recovery_used` at `warning`, notifies the owner through the new essential-group `security.recoveryCodeUsed` event, and leaves `lastTotpStep` untouched so the TOTP replay floor (#865) is unaffected. `POST /api/account/2fa` gains `regenerate-codes` — deliberately *not* gated on a fresh authenticator code, since the person who needs it most is the one who just signed in with a recovery code — `disable` clears the set, `GET` reports remaining/total counts and never a code, and the account card shows the set once with copy/download plus a permanent remaining count. Impersonation is refused exactly as for the existing 2FA mutations. The format rules (unambiguous 27-character alphabet, dash-insensitive normalisation, and the shape test that keeps a 6-digit code out of the recovery bucket) live in the dependency-free `src/lib/recoveryCodeFormat.ts` under a 95% coverage floor; the sign-in-and-no-reuse promises are covered by `e2e/two-factor-recovery.spec.ts`. Design notes: `docs/two-factor-recovery.md`.

## [0.200.0-beta] - 2026-09-14

_Shipped 2026-09-14 04:14 UTC · commit [c0a5e5e](https://github.com/21072026/Internship/commit/c0a5e5e1fa41d2638bf957faaf0dff975940bb29)_

- **The plan's project cap is enforced, not just displayed** (#2273). `orgPlans.maxProjects` (FREE 3, PRO 25, ENTERPRISE unlimited) had been declared since #547 and read by exactly one thing — the usage gauge on `/admin/organizations` — so `POST /api/projects` never checked it. #2270 then opened project creation to the largest role population there is (MENTEE), which turned unbounded growth per tenant from theoretical into the default; the per-user rate limit added there is a brake, not a quota (its default counter is an in-process Map that resets on redeploy and is not shared across replicas). `checkProjectLimit()` joins `checkActiveRelationLimit()` in `src/lib/planGate.ts`, counting through the new `countOrgProjects()` in `src/lib/metering.ts` so the gate and the gauge read the same number from the same file (#1750's rule) — every project counts, whatever its status, exactly as the gauge counts them. The refusal is a 403 under its **own** code `project_limit_reached` rather than the relation gate's `plan_limit_reached`, because `mentorshipAssignmentError` maps that code to a sentence about active mentorships and would translate a refused project into "you have too many mentees"; the project form renders the new code from the dictionary in EN/TR/DE instead of echoing the server's English literal. Fails open when no org resolves and when the plan is unlimited, so the grandfathered ENTERPRISE `default` org is untouched — asserted, along with the FREE refusal, in `e2e/plan-limit-gate.spec.ts`.

## [0.199.0-beta] - 2026-09-14

_Shipped 2026-09-14 04:14 UTC · commit [85ad445](https://github.com/21072026/Internship/commit/85ad44564b1d95dedcabfa1b0fa642670535efc2)_

- **Job queue engine — `JobRun`, `enqueue()`, a `SKIP LOCKED` `claim()`, lease reaping and a run ledger** (#1671). The durable substrate the worker loop (#1673) and the schedule registry (#1676) plug into; nothing runs a job yet. `model JobRun` joins the `Job` model that shipped with #1674 — one row per *attempt*, carrying `trigger` provenance (`'schedule' | 'manual' | 'boot'`, so the operations console can tell "the 09:00 slot fired" from "an admin pushed re-run"), a counters-only `summary`, `durationMs`, `ok` and the error message — related `onDelete: Cascade` and deliberately NOT registered in `TENANT_MODELS`, because it carries no `orgId` and is system-level operational data reached only through its `Job`. `src/lib/jobs/queue.ts` holds the rules and `src/lib/jobs/queueStore.ts` the Prisma row access, the same split (and for the same testability reason) as `lease.ts`/`leaseStore.ts`. `enqueue()` returns the **existing** row on an `idempotencyKey` collision instead of throwing — any other insert failure still throws, and a collision whose row cannot then be found rethrows the original cause rather than reporting a phantom success. `claim(workerId, {limit, leaseMs})` is one `$transaction` doing `SELECT id … WHERE status='PENDING' AND runAt <= ? ORDER BY priority DESC, runAt ASC LIMIT ? FOR UPDATE SKIP LOCKED` and then an `UPDATE … SET status='RUNNING', lockedAt, lockedBy` over exactly those ids, so two workers racing get disjoint sets; it does **not** touch `attempts` (that is #1673's bookkeeping, and incrementing on a claim would burn a retry on a container that died before running anything). **Requires MySQL 8.0.1+** for `SKIP LOCKED` — the server runs 8.0.46 and CI the `mysql:8.0` image; an older engine fails loudly as a syntax error rather than silently serialising the queue. `reapExpiredLeases(leaseMs)` returns a stale `RUNNING` row to `PENDING` with `attempts` untouched, because a lease that expired is not an attempt that failed. `recordRun()` opens the ledger row **before** the handler (a job that wedges its container is exactly the one that needs a row) and stamps `finishedAt`/`durationMs`/`ok`/`summary` after; **every ledger write is wrapped in try/catch** so a broken ledger can never fail the work it observes, while the handler's own throw is always recorded *and* rethrown for the worker and the error tracker (#1600). `summary` is `Record<string, number>` in TypeScript and re-checked by `sanitizeSummary()` at runtime, so an address, a name or a message body cannot reach the admin surfaces that render it near-verbatim. `pruneJobRuns(retentionDays = 90)` cuts by `startedAt`, not `finishedAt` — a run that never finished would otherwise be the one row the sweep could never remove. Delivery is **at-least-once** and the module doc comment says so: handlers must be idempotent. `src/lib/jobs/types.ts` names the jobs in a union with typed payloads; #1677 adds the twelve cron callbacks to it.

## [0.198.0-beta] - 2026-09-14

_Shipped 2026-09-14 03:40 UTC · commit [55d37bf](https://github.com/21072026/Internship/commit/55d37bf088e0a602fdeb5e503dc21cb7b66d63e1)_

- **Admin-side 2FA reset** (#1543). `POST /api/admin/users/[id]/reset-2fa` clears a locked-out user's TOTP secret, replay high-water mark and recovery codes, stamps `sessionsValidFrom` and revokes their trusted devices in one transaction. The decision is the unit-tested rule in `src/lib/twoFactorResetRule.ts`: the caller's ADMIN capability is re-read from the database (not the 12h JWT), an impersonation session is refused, and any ADMIN target — peer or self — is refused. AuditLog + ActivityLog rows are written inside the same transaction, so a reset that cannot be recorded does not happen; the owner is told in-app and by e-mail, naming the admin.

## [0.197.5-beta] - 2026-09-14

_Shipped 2026-09-14 03:37 UTC · commit [a4351a7](https://github.com/21072026/Internship/commit/a4351a7911fae0ebf673abba1269f7430928c97d)_

- **Notification caps are counted per person, not per relation** (#2287). The dormant check-in budget and the weekly-report reminder were both capped on the `MentorshipRelation`, so a mentee holding two ACTIVE relations held two independent budgets — four "still interested?" mails where `docs/dormant-first-contacts.md` promises two, the second of which already says nobody will write again, and two identical report reminders every week forever. `sendDormantCheckIns` now decides on the person: the **sum** of `dormantNudgeCount` across their ACTIVE relations is the spend, and the **latest** `dormantNudgeSentAt` across them is the clock the 31-day spacing is measured from (the in-memory Set added in #2283 deduplicated one tick only, so the extra mail was deferred to the next run rather than cancelled). No column and no migration: both figures come from rows that already exist. `sendWeeklyReportReminders` collapses to one relation per mentee **before** the claim, picked deterministically by lowest id so two overlapping ticks race on the same `@@unique([relationId, weekStart])` row, and the new nullable `WeeklyReportReminder.recipientId` (plus `prisma/backfill-weekly-report-reminder-recipient.mjs`, wired into the deploy) makes "has this human been reminded this week?" answerable at all. The `@@unique([recipientId, weekStart])` that would make it structural is deliberately NOT in this change — deploys run `db push` before the backfills, so the index would be built against still-NULL rows and against exactly the duplicates at issue; it is the contract step, gated on the backfill's duplicate count reading zero on prod and shared preview, the same sequencing as #2286. Webhook and audit fan-out stay per relation, and the two mentor-facing sweeps (`checkMentorInteractionReminders`, `checkStageDeadlineReminders`) are documented as correctly per relation: their recipient is the mentor and each message is about a different mentee, so collapsing them would drop a deadline rather than a duplicate. With #419 holding, every number above is identical to today's — this is defence in depth, and a prerequisite if #1799 ever raises the per-mentee limit above one on purpose.

## [0.197.4-beta] - 2026-09-14

_Shipped 2026-09-14 03:30 UTC · commit [5da215b](https://github.com/21072026/Internship/commit/5da215b8080f503e63046dd3d66aaa64f12e6784)_

- **A non-internship vertical no longer greets its users as "Internship CRM"** (#2355 follow-up). The brand wordmark falls back to the product name only for the INTERNSHIP vertical; a MARKETING (or any future) organization that has set no white-label brandName now falls back to its OWN name instead — `getOrgBranding()` reads the org's vertical and name and passes the name as `resolveBranding()`'s new `fallbackName` when the vertical is not INTERNSHIP. A tenant that has configured white-label branding is unaffected (a set brandName still wins); the INTERNSHIP product default is unchanged. Fixes a marketing tenant seeing the internship wordmark on its own admin panel after the marketing.ersah.in cutover.

## [0.197.3-beta] - 2026-09-14

_Shipped 2026-09-14 03:27 UTC · commit [f8146d1](https://github.com/21072026/Internship/commit/f8146d1a9aec05c449461096d56f45e02d4d7f08)_

- **The service worker no longer caches authenticated responses, and now registers for signed-out visitors** (#1550). Two defects in `public/sw.js`. (1) SECURITY: the v3 worker wrote *every* same-origin GET to one shared cache — authenticated JSON included — with no per-account scoping and no purge on sign-out, so on a shared device an offline navigation could be answered out of another account's response. The fetch handler now decides per request rather than per route (a route list only covers the routes that existed when it was written): nothing under `/api/`, nothing carrying an `Authorization` header, and nothing the server marked `Cache-Control: private/no-store` or `Vary: Cookie` is ever written — which keeps authenticated *documents* out too — and the same gate applies on the read path, so a stale entry left by an older worker is not served either. `CACHE` is bumped to `internship-crm-v4` so `activate` drops what installed clients already hold, and a new `PURGE_CACHE` message empties every cache (re-seeding only the public `/offline` + icon precache); `signOutEverywhere()` posts it and awaits the ack before redirecting, deleting the caches from the page itself when no worker answers. The `/api/realtime/` and `text/event-stream` exclusions from #1464 are untouched, as is push. (2) PWA: registration lived in `InstallAppButton`, which is only mounted in the signed-in role sidebars, so public pages got no worker, no offline fallback and no install prompt — it moves to `ServiceWorkerRegistrar`, mounted from the root layout. Covered by `scripts/test/service-worker-cache.test.mjs` (the fetch handler evaluated in a fake worker scope — no browser, runs on every PR) and `e2e/sw-cache-scoping.spec.ts` (a live worker: registers while signed out, no `/api/` entry, sign-out empties the cache).

## [0.197.2-beta] - 2026-09-14

_Shipped 2026-09-14 03:14 UTC · commit [0431dbb](https://github.com/21072026/Internship/commit/0431dbb6c5f52fbf6931b5dd70b7365eb1341a68)_

- **Infra: requisition backfill ordering** (#1385). Run the topic deploy's existing requisition backfill after demo seeding and the organization backfill so newly seeded legacy needs and repaired company orgIds are included. Preserve non-blocking execution and stdout/stderr logs; production and shared preview already run the backfill after organization repair.

## [0.197.1-beta] - 2026-09-14

_Shipped 2026-09-14 03:14 UTC · commit [a75cd07](https://github.com/21072026/Internship/commit/a75cd07188bb7199ac12d0ca10e89453c367f465)_

- **Follow-ups from the review of mentee-owned projects** (#2270). Four holes the shipped change left behind. (1) The project-level IP gate is now withheld from a MENTEE server-side, symmetrically with `isPublic`: `POST /api/projects` ignores `contributorTermsRequired` / `contributorTermsKey` from a mentee creator, and `PUT /api/projects/[id]` refuses either field (plus `isPublic`) from a mentee owner with 403 `programme_only`. Only the portal form hid the picker before, so a hand-rolled `{"contributorTermsRequired": false}` created a project nobody joining it was ever asked to accept terms for — and one that dropped out of the admin acceptance report, which counts `contributorTermsRequired: true` rows only. (2) `canManageMembers()` in `[id]/members` now honours the legacy `ownerUserId` pointer, like `isProjectOwner()` and like the `isLead` test on `/projects/[id]`: it read OWNER `ProjectMember` rows only, so on every topic/preview environment (the demo seeder sets `ownerUserId` alone and no deploy script runs `backfill-project-members.mjs`) the demo owner got the members panel rendered and a 403 from every fetch behind it — a silently empty roster. (3) `ProjectForm` is keyed on the edit target in `ProjectsManager`, so switching pencils with the form open no longer saves project A's values (owner included) onto project B — the extracted form seeds its state once, where the old inline form re-populated it in `edit()`. (4) The advice given to a mentee owner is now true: a private project cannot receive join requests (`not_public`), and only an admin can publish a mentee-created one, so the members panel says so for a private project (new `projects.ownerInviteAskAdmin`, EN/TR/DE), the `mentee_owner_invite` 403 names that step, and the `menteeOwnProject` feature-catalogue copy no longer promises a join-request flow that cannot occur while the project is private. No schema change.

## [0.197.0-beta] - 2026-09-11

_Shipped 2026-09-11 17:54 UTC · commit [7e319d7](https://github.com/21072026/Internship/commit/7e319d74d300a2463c86e5823e2a1d095ec49a3d)_
...
## [0.196.0-beta] - 2026-09-11

_Shipped 2026-09-11 17:05 UTC · commit [3928545](https://github.com/21072026/Internship/commit/3928545c727d1d442d5cb5e315bc3ddc38654c54)_

- **Broadcast and fan-out surfaces are tenant-scoped** (#2357, epic #2349). `Webhook`, `Announcement` and `Newsletter` gain a nullable `orgId` (backfilled to the default org by the schema-derived deploy backfill) and are registered with the tenant middleware — closing a latent cross-tenant leak that only bites once a second organization exists: `dispatchWebhook` filtered on `active` alone, so one tenant's `pipeline.stage_change` would fan out to another tenant's endpoint, and the newsletter dispatcher selected recipients by role across every tenant. Now `dispatchWebhook(event, data, orgId)` scopes to the acting tenant (threaded through all ten call sites — the acting user's org, or the relevant relation/mentor for the public and background paths), the newsletter dispatcher scopes recipients to the issue's own `orgId`, every create path stamps the creator's org (the curated-library cron issues to the default org), and the admin webhook endpoints (list, edit, delete, read) filter by the caller's org so one tenant's admin can neither see nor touch another's endpoints. The scoping is explicit rather than left to the middleware because these paths run fire-and-forget or from the cron, outside any request scope, and the middleware is advisory until `MT_ENFORCE_ISOLATION`. A no-op for today's single-tenant product (everything is the default org); a two-tenant e2e proves a tenant admin sees, edits and deletes only its own webhooks and that a created webhook is stamped with its org.

## [0.195.0-beta] - 2026-09-11

_Shipped 2026-09-11 13:17 UTC · commit [79a773e](https://github.com/21072026/Internship/commit/79a773eafed4bdd7884a42518cc86be991503087)_

- **Per-vertical terminology overlay** (#2354, epic #2348). coreCRM serves several products from one dictionary, and a MARKETING tenant should not read "candidate" and "mentee" where it means "lead". A vertical may now override individual dictionary strings through a `DeepPartial` overlay deep-merged onto the base translation (`src/i18n/verticalOverlays.ts`), resolved per request from the signed-in user's `Organization.vertical` and applied in both `getServerDictionary()` and the root layout's client dictionary so server render and client payload agree. INTERNSHIP overrides nothing — its overlay is empty in every locale, so the merge returns the base object unchanged (referentially identical) and the whole layer is invisible to today's product; a signed-out visitor resolves to the default with no query, so the overlay costs the live single-tenant product nothing. The first overrides dress the candidates surface (`nav.candidates`, `candidates.title`, `candidates.subtitle`) as Leads in EN/TR/DE; the set grows as more marketing surfaces are dressed. Overlays only ever REPLACE a leaf string that already exists in the base — a unit spec asserts INTERNSHIP stays empty, every overlay key exists in the base (a typo is a build failure, not a silent no-op), and a MARKETING e2e proves a marketing admin sees "Leads" while an internship admin still sees "Candidates" on the same page.

## [0.194.0-beta] - 2026-09-11

_Shipped 2026-09-11 12:39 UTC · commit [b64d811](https://github.com/21072026/Internship/commit/b64d811a048beab6b10feb64a46851f7e5b083bc)_

- **MARKETING vertical starts on a sales funnel, seeded at org creation** (#2353, epic #2348). A new `MARKETING_FUNNEL` program template (`src/lib/programTemplates.ts`) — a seven-stage lead-to-deal pipeline (New lead → Contacted → Qualified → Proposal → Negotiation → Won/Lost) with its own EN/TR/DE labels, its own follow-up cadence and SLAs, and its own keys (`LEAD_*`, `DEAL_*`, never a canonical internship key) — is provisioned into a new MARKETING organization's `PipelineStage` rows at creation. `defaultTemplateForVertical()` maps a vertical to its starting template; `provisionStagePreset()` applies it, and an INTERNSHIP org maps to nothing so it starts on the canonical built-ins with no rows written, exactly as before verticals existed. Provisioning runs server-side at creation, deliberately bypassing the stage editor's FREE-plan gate (seeding a tenant is a super-admin act, not a tenant self-service edit). The stage write itself is now one shared `replaceStages()` used by both org provisioning and the admin stage editor, so the two can never drift in how a row is shaped. A no-op for today's product; a MARKETING e2e asserts the funnel lands and an INTERNSHIP e2e asserts nothing is written.

## [0.193.0-beta] - 2026-09-11

_Shipped 2026-09-11 12:22 UTC · commit [3a86774](https://github.com/21072026/Internship/commit/3a86774ff4ad650cdac5b1a8bd19c18b63113b3d)_

- **Write-path capability gate on the mentorship modules** (#2352, epic #2348). Hiding a module from the sidebar (#2351) is not access control — the route is still reachable by a direct POST — so this closes the actual hole: every mutating handler of a mentorship-specific module (`/api/goals`, `/api/weekly-reports`, `/api/questions`, `/api/meeting-requests` and `/api/evaluations`, POST plus the `[id]` PATCH/DELETE — twelve handlers in all) now calls `requireCapability(session.user.orgId, …)` before any write, and a tenant whose vertical does not carry the module is refused with `403 { code: 'capability_unavailable', capability }`. The gate runs before validation, so a gated vertical never even reaches the 400. The acting user's own org decides it, INTERNSHIP carries every capability, so this is a strict no-op for today's product — proven by an INTERNSHIP e2e that posts to all five and asserts none is capability-gated, alongside a MARKETING e2e that asserts all five are.

## [0.192.0-beta] - 2026-09-11

_Shipped 2026-09-11 11:58 UTC · commit [6460db5](https://github.com/21072026/Internship/commit/6460db5221a47ba06d50c89986db29afbb195c31)_

- **Vertical capability gate on the shell and sidebar** (#2351, epic #2348). A tenant's `Organization.vertical` now decides which modules its shell shows: `src/lib/verticals.ts` already declares each vertical's capability set, and this wires it into the navigation. Capability-tagged nav links (`NavLink.capability`) survive only for an org whose vertical carries that capability — the mentorship, placement and sourcing admin links (`/admin/mentors`, `/admin/mentorship`, `/admin/mentor-applications`, `/admin/mentee-activity`, `/admin/offers`, `/admin/requisitions`, `/admin/interview-requests`, `/interviews`, `/admin/sources`) fall away for a vertical that lacks the module, while every untagged link is always shown. `visibleNavLinks()` does the filtering (pure, unit-tested); the three nav components and the command palette take the resolved capability set as a prop, resolved server-side per request by `shellCapabilities(orgId)` in the layout. The `/mentor` and `/portal` shells additionally redirect a vertical that has switched the mentorship module off entirely. INTERNSHIP carries every capability, so all of this is a strict no-op for today's product — the same sidebar, the same shells, proven by an INTERNSHIP e2e that asserts nothing is hidden. The write-path gate (a hidden module must also refuse a direct POST) is the next slice, #2352.

## [0.191.0-beta] - 2026-09-11

_Shipped 2026-09-11 10:51 UTC · commit [ca90295](https://github.com/21072026/Internship/commit/ca90295a02f3cf08bf3b376f6b87f42ab6fcbc8c)_

- **`Organization.vertical` — the foundation for coreCRM's verticals** (#2350, epic #2348). The app is becoming one core serving several products (internship CRM today, a marketing CRM next), and which product a tenant sees is now a property of the *tenant*, not of the deployment: a `vertical` column on `Organization` (literal default `INTERNSHIP`, so `db push` fills existing rows itself), a frozen catalogue in `src/lib/verticals.ts` declaring each vertical's capabilities, and `verticalFor(orgId)` / `verticalsFor(ids)` in `src/lib/verticalContext.ts`. The read ALWAYS takes an explicit `orgId` and is never put in the JWT — `role` is in the token and the schema's own SUPER_ADMIN note explains what that costs, and most of the surface that will need a vertical later (cron sweeps, newsletter fan-out, webhook dispatch) has no session to read. Super admins can set a vertical when creating a tenant and change it on `/admin/organizations`; a tenant admin cannot change their own, the same rule the plan already carries. Nothing reads the catalogue yet — no capability is gated, no label changes, no route moves — so this ships with zero behaviour change by construction, and a unit test pins the ways that could stop being true: an unknown key must fall back to `INTERNSHIP` rather than to an empty capability set, and `INTERNSHIP` must remain a superset of every capability any vertical declares. The per-vertical stage preset is deliberately NOT in this slice: it would be a plain-string key into `PROGRAM_TEMPLATES` that nothing here resolves, so a wrong key would type-check, pass every test and only surface as an empty pipeline in the slice that finally reads it — it lands in #2353 with the template it points at, a reader, and an e2e.

## [0.190.0-beta] - 2026-09-09

_Shipped 2026-09-09 21:43 UTC · commit [be597bb](https://github.com/21072026/Internship/commit/be597bb1fdeb9375d5ef7d1e5043ecfa472b15e0)_

- **Enforced SSO per tenant — every other door closes, with a break-glass admin and a session sweep on flip** (#1950). `Organization.ssoEnforced` makes "our users cannot sign in with a password" a claim we can actually stand behind: the rule lives once in `src/lib/ssoEnforcement.ts` and **every** password-derived path calls it — the credentials provider, "remember me" silent re-authentication (refused in `POST /api/auth/remember/refresh` *and* in the `remember` provider, since the grant is a 60-second bearer token), password reset completion for both `RESET` and `SET_INITIAL`, invitation/self registration (it sets a password) and the self-service password change; `/api/auth/forgot` stops mailing a link that would be refused, and the admin reset-password route says why instead of looking like broken SMTP. Impersonation is deliberately left open and the reasoning is written down in `docs/sso-saml.md`. Two interlocks make it safe to turn on: enforcement cannot be enabled unless `isSsoActive(org)` **and** at least one active `ADMIN` holds the new `User.ssoExempt` break-glass exemption (granted through `POST /api/admin/users/[id]/sso-exempt`, admins only, audited as `sso.exempt_granted`/`sso.exempt_revoked`, badged in Admin → Users, and the last one cannot be revoked while enforcement is on), and enforcement only *applies* while SSO is still active, so a plan downgrade or a broken config falls back to password login instead of locking a tenant out. Flipping it on sweeps the tenant in batches of 500 — `sessionsValidFrom` **and** `revokeAllTrustedDevices`, never one without the other — and writes one `sso.enforced` activity row with the count. A refused sign-in returns the new allow-listed `SSO_REQUIRED` provider error, so the sign-in page says "your organization requires single sign-on" and links to `/auth/sso` instead of "invalid email or password"; that narrow enumeration trade is recorded in `docs/security-exceptions.md`. `e2e/sso-enforced.spec.ts` (`@smoke`) refuses each door by calling the API/provider directly, with a passing control beside each refusal.

## [0.189.0-beta] - 2026-09-09

_Shipped 2026-09-09 21:35 UTC · commit [b7563de](https://github.com/21072026/Internship/commit/b7563de89ec5ad646bedf4965f500d4897e7f07c)_

- **Group calls stop dying at five minutes — honest routing, and we count our own JaaS allowance** (#2011). The video host used to be picked by `inviteeCount === 1`: the JaaS tenant was reserved for pairs to protect a 25-monthly-active-participant free allowance nobody in the codebase could measure, so every project meeting, every bulk schedule and — via `inviteeCount: null` — every recurring series was routed *by construction* to public `meet.jit.si`, which hangs up an embedded call after about five minutes, with nothing warning anyone it was about to happen. Three changes. (1) `resolveMeetingLink()` replaces the four direct `generateMeetingLink()` calls and routes any room — 1:1, group, bulk, series — to the tenant while its projected head-count still fits the allowance; a spent allowance degrades a 1:1 to the public instance rather than failing to start. (2) `FreeRoomWarning` renders above the embedded call whenever the room is a `meet.jit.si` one, before anyone joins, in EN/TR/DE, naming the cutoff and the escape from it (the same room in a browser tab has no limit, and the button is in the warning). (3) The allowance is counted from data we own: `JaasMonthlyParticipant` is one row per (UTC month, participant) written from the existing `PARTICIPANT_JOINED` webhook — an HMAC of the participant id and nothing else, no names, no join to a `User` — surfaced on `/admin/integrations` as "N / 25 monthly active participants" via `GET /api/admin/integrations/jaas-usage`. The limit is `JAAS_MONTHLY_ACTIVE_LIMIT` (default 25, `0` = never use the tenant), not a literal in a component. Nothing here gates a call: video stays free-core, over-allowance is an announced degradation, and an unset tenant, a broken key or an unreadable database all resolve to a public room. `src/lib/meetingRoom.ts`'s reasoning comment now describes the shipped rule instead of the retired one.

## [0.188.0-beta] - 2026-09-09

_Shipped 2026-09-09 21:35 UTC · commit [e3c60b0](https://github.com/21072026/Internship/commit/e3c60b080c23b00613ca21f5e2f66a08ff70165f)_

- **A meeting can finally be moved, called off or deleted** (#1980). `/api/meetings/[id]` had no `PATCH` and no `DELETE`, so the only way to fix a wrong time was to schedule a second meeting and abandon the first — which stayed on the dashboard banner, in the reminder queue, in the calendar, in the `.ics` feed and on the participant's real Google Calendar forever. `Meeting` gains `status` (`MeetingStatus { SCHEDULED | CANCELLED }`), `cancelledAt` / `cancelledById` / `cancelReason`, `rescheduledAt` / `previousScheduledAt` so "this moved" is answerable without an audit join, and `batchKey` (indexed) — the shared identity of one bulk schedule, written by `POST /api/meetings`, so a cancel can mean this invitee (`scope: 'one'`, the default) or the whole session (`scope: 'batch'`); a null key means just this row, never "all the nulls". `PATCH` moves the time (re-parsed through `parseUserDateTime` against the actor's zone, never a bare `new Date()`, #1061), the clock, the title and the room link, clears `reminderSentAt` so the new time gets its own reminder, stamps the old time, and resets an RSVP that was given to the time nobody is meeting at any more; `status: 'CANCELLED'` records who called it off, when and why. `DELETE` removes the row — and what was *written* in the meeting survives, because `PersonalNote.meetingId` and `InteractionLog.meetingId` are `onDelete: SetNull` on purpose. Both refuse a meeting whose `endedAt` is set (410) and `PATCH` refuses one already cancelled (409); a cancelled row stays deletable, which is the only way a mistake ever leaves the list. Authorization is server-side and fail-closed in one place — the new `canManageMeeting()` in `src/lib/meetingAccess.ts`: organiser, the relation's mentor, admin. A MENTEE is a participant, not the owner (they decline; counter-proposals are #1982), and a COMPANY or SOURCE session never reaches the rule at all — every refusal answers exactly like an unknown id, so the id space stays opaque. Cancelling and deleting call `removeMeeting()` (its first real call site; #1986 owns the rest) before the rows go, since `GoogleCalendarEventLink` cascades away with the meeting; a move re-pushes, which PATCHes the event already on the invitee's calendar instead of leaving a second one next to the stale first. Both verbs are rate-limited (`meeting-mutate`, 20/min), audited (`meeting.cancel` / `meeting.delete`), and `meeting.rescheduled` + `meeting.cancelled` join `WEBHOOK_EVENTS`. Every reader that answers "is there a meeting?" now filters CANCELLED out: the dashboard banner, the reminder cron, `/api/calendar-events`, the ICS subscription feed, the public per-meeting `.ics` (410) and the auto-log sweep — a meeting that was called off did not take place, and writing "the scheduled meeting took place" into a mentee's history would be a false record. `e2e/meeting-reschedule.spec.ts` covers schedule → move → cancel, the mentee's and an unrelated mentor's 404, batch vs single scope, and a delete that keeps the note and the log.

## [0.187.1-beta] - 2026-09-09

_Shipped 2026-09-09 13:24 UTC · commit [ccde937](https://github.com/21072026/Internship/commit/ccde9377ddf7312341f15e8a2d30656b877e7637)_

- **One id per request, from the middleware to the log line** (#1601). `src/middleware.ts` mints (or honours, within a bounded alphabet) an `x-request-id`, forwards it to the handler and echoes it on every response — error responses included. `src/lib/requestContext.ts` carries it in an `AsyncLocalStorage` modelled on `orgContext.ts`, and `logger.ts` now stamps `requestId` (and `orgId`) onto every line automatically, so a support ticket can be traced to the lines it produced.

## [0.187.0-beta] - 2026-09-09

_Shipped 2026-09-09 13:20 UTC · commit [717d151](https://github.com/21072026/Internship/commit/717d151e143c6bd63265d696b347832c8e651982)_

- **Convert a company enquiry into a Company and a COMPANY login in one action** (#1863). `POST /api/admin/company-inquiries/[id]/convert` (ADMIN, inside `withTenantScope`) creates the `Company` and mints the login's `InvitationToken` in a single `prisma.$transaction`, so a half-finished conversion can never leave a Company nobody can sign in to. The login goes through the **existing invitation flow** — `InvitationToken` gains a `companyId` that registration attaches to the new COMPANY user, `createInvitation` accepts the COMPANY role, and no password is ever set by an admin nor any credential mailed that an admin could read off their own screen; the token is emailed and never returned in the response. Idempotence is a conditional `UPDATE` on the new `CompanyInquiry.convertedCompanyId` inside the transaction, so a double-click (or two admins on one row) produces one Company and one invitation and the loser gets `409 already_converted` naming what the enquiry already became; an address that already has an account gets `409 email_taken` naming its company, and an address holding a **live invitation** gets `409 invitation_pending` naming the company that invitation is for — the user lookup cannot see that case, and without it a second enquiry from the same address would mint a second Company whose ownership is then decided by whichever token is redeemed first (a revoked invitation is not a live one, so withdrawing it makes conversion possible again, exactly as it does re-inviting, #2071). Registration refuses a COMPANY invitation whose company has since been deleted rather than failing on `User.companyId`'s foreign key with an error the invitee cannot act on. Both new rows carry the acting admin's `orgId` explicitly (default-org fallback, as registration does) rather than relying on the middleware. The enquiry is never deleted — it is stamped `CLOSED` + `convertedCompanyId`/`convertedAt` and the row links to the company it became. `emailSent` is derived from what the transport reported, and a failed send says so in the UI and in the `company.inquiry.converted` audit line. `inviteCreate.ts` is split into `persistInvitation` / `logInvitationCreated` / `deliverInvitation` (with `createInvitation` still those three in order) so the conversion can mint inside a transaction without an SMTP round-trip holding it open, and without a second InvitationToken minting path.

## [0.186.0-beta] - 2026-09-09

_Shipped 2026-09-09 12:57 UTC · commit [e642f20](https://github.com/21072026/Internship/commit/e642f2094b652bed1ea37bbf3ec13d5efd18c6ba)_

- **Per-org broadcast sending quota** (#1754). Announcement e-mails and newsletter issues now share one monthly recipient band per organization: `min(plan's monthlyBroadcastRecipients, the new broadcastMonthlyRecipients setting)`, over the calendar month in UTC. `src/lib/broadcastQuota.ts` is the single source — the metered set is the one exported `BROADCAST_EMAIL_CATEGORIES` constant (`announcement`, `newsletter`), month-to-date usage is derived from rows that already exist (`NewsletterSend`, `Announcement.emailedCount`) rather than a counter that can drift, and the check runs before the audience is expanded so an over-band send is refused whole with a 403 naming limit/used/requested/remaining/resetsAt. `dispatchNewsletter` evaluates the quota before claiming the issue and never meters a resume (a run already in `SENDING` was authorised once and its delivered half is already inside the window it would be counted against), and a refusal DISARMS the issue instead of leaving it armed — a refused send-now goes back to `DRAFT` with `scheduledAt` cleared, so the cron cannot mail unattended something the admin was told had not been sent, while an issue the admin had deliberately scheduled for a date keeps that date (the 403 carries `status` and `stillScheduled` so the composer can say which happened); `GET /api/admin/broadcast-quota` feeds a "used X of Y this month" line on both composers. The operator setting can only tighten a plan's band, never lift it. Free core is untouched and asserted with the band at zero: 1:1 messages, notifications, verification/invitation/reset mail, meeting invitations and reminders, digests, dormant check-ins, in-app-only announcements and newsletter test copies are never metered.

## [0.185.1-beta] - 2026-09-09

_Shipped 2026-09-09 12:45 UTC · commit [4e9b225](https://github.com/21072026/Internship/commit/4e9b2251ebde9b44cbcf5faedf7a39335605e6ac)_

- **Every tenant-keyed model is now auto-scoped** (#1559). `Tag`, `StageSla`, `PipelineStage`, `CompanyInquiry`, `Offer`, `InterviewPanel`, `EvaluationTemplate` and `InvitationToken` carried an `orgId` without being registered in `TENANT_MODELS`, so they were scoped only by hand-written `where` clauses; all eight are registered and the `check:tenant-models` ratchet is empty (`EXPECTED_PENDING = 0`). Every create path was audited: the public `/for-companies` enquiry now stamps an org instead of leaving it NULL (a NULL-org row would vanish from the wrapped admin triage list), `/api/invite/[id]` and `/api/admin/evaluation-templates` are wrapped in `withTenantScope`, and the admin offer list's `isIsolationEnforced()`-gated `orgScoped()` filter is gone as redundant with the middleware. The sessionless token paths (register, invite-opened, verify-email), the offer-expiry cron and the super-admin pipeline-stage route stay unwrapped by design and now say so. New `e2e/tenant-models-registered.spec.ts` proves cross-tenant reads return nothing for each of the eight, in both directions, with an enforcement-off control. `MT_ENFORCE_ISOLATION` is unchanged (#1572).

## [0.185.0-beta] - 2026-09-09

_Shipped 2026-09-09 10:24 UTC · commit [1829411](https://github.com/21072026/Internship/commit/1829411f47dc0daef896791c3b550cdbdf0cf115)_

- **The roster ingestion engine** (#1965). `src/lib/importPreview.ts` is now the one import engine in the tree — `runImport({ parse, validate, resolve, apply })`, the `RowStatus`/`RowResult`/`ImportReport` contract and a shared delimited parser that reads quoted embedded newlines, CRLF, a UTF-8 BOM and `;`-delimited German Excel exports. `src/lib/rosterIngest.ts` is its first consumer: HTTPS transport through `assertPublicHttpsUrl()` with a size cap and a pinned SFTP host-key check, a SHA-256 file hash that makes an unchanged file a no-op, a pure `diffRoster()` (create / update / unchanged / absent, keyed on e-mail or an external id) and chunked apply. `src/lib/rosterIngestStore.ts` persists `RosterFeed`/`RosterRun`/`RosterRowResult` (all three tenant-scoped and registered in `TENANT_MODELS`) with one transaction per chunk and a contiguous `nextChunkIndex` checkpoint, so a crash mid-batch resumes instead of re-applying; `src/lib/externalSyncPolicy.ts` is the single do-not-clobber-user-edits policy shared with SSO sync. The dry run is the same code path with a writer that does not write. 30-case unit suite (`npm run test:roster-ingest`, wired into CI) covering the parser fixtures, the diff, resume-after-crash, idempotency and two-tenant isolation. Deprovisioning (`absent[]`) is computed and reported only; acting on it stays #1966.

## [0.184.0-beta] - 2026-09-09

_Shipped 2026-09-09 10:05 UTC · commit [223a477](https://github.com/21072026/Internship/commit/223a477897df47ba10193965df1088d06bd2c768)_

- **One notification router, one typed event catalogue and a per-recipient delivery ledger** (#1710). Notifications fanned out per feature: each send site picked its own event string, decided on its own whether an e-mail went with the in-app row, and reached for whichever preference helper its author remembered — and "was the mentor actually told, on which channel?" had no answer anywhere. `src/lib/notifications/catalog.ts` now carries one typed entry per event (all 102 `notifications.events` keys) with its notification category, e-mail group, default channels, `immediate|batched` delivery mode, link kind and declared template placeholders, and `notifyEvent(recipients, eventKey, payload)` in `src/lib/notifications/router.ts` resolves recipient → locale → preferences → channels → ledger row → channel. An unknown event key and a payload missing a placeholder are both compile errors (`typeContract.ts` asserts that with `@ts-expect-error`, which `tsc --noEmit` fails when the mistake stops being one), and `scripts/test/notification-catalog.test.mjs` fails when a dictionary key has no catalogue entry, an entry has no template, or a template's placeholders drift from the declared payload **in any one of the three locales** — the failure that renders a literal `{name}` to German readers only. The new `NotificationDelivery` model is one row per (recipient, event, channel): `QUEUED|SENT|SKIPPED|FAILED` with a `skipReason`, `orgId` stamped from the recipient's own row and registered in `TENANT_MODELS`, and a non-null globally-unique `dedupeKey` (nullable unique columns de-duplicate nothing in MySQL — the trap already documented on `Job.idempotencyKey`) so a retried send neither duplicates the row nor sends twice. The preference check runs **before** the row is claimed, which is the opposite of the dormant-first-contact sweep's recorded mistake of spending a person's mail quota on mail that was never sent. `notifyEvent` never throws and never blocks the caller's transaction: every step degrades to a log. Nothing is migrated in this change — the existing `notify()` keeps its exact in-app-only behaviour and is re-exported from the router as the shim its existing call sites move through, and the e-mail channel enqueues a durable `notification.email` job rather than sending inline (its handler arrives with the queue worker).

## [0.183.0-beta] - 2026-09-09

_Shipped 2026-09-09 10:04 UTC · commit [3b27128](https://github.com/21072026/Internship/commit/3b271281769352344088dabafa8ea303c0e3abd6)_

- **Starter skill vocabulary with aliases and adjacency** (#1816). `src/lib/skillSeed.ts` holds ~100 curated skills, each with a stable key, EN/TR/DE labels, the spellings people actually type (`ReactJS`/`React.js` → `react`, `k8s` → `kubernetes`, `Postgres` → `postgresql`, `Çevik` → `agile`) and unweighted near-neighbour links (`react` ↔ `frontend`, `docker` ↔ `devops`), grouped into 13 categories. Grown from the CV parser's `SKILL_VOCABULARY` — which every entry of is covered — plus the umbrella terms it lacked (Frontend, Backend, Full Stack, Mobile, DevOps, Data Engineering, QA/Testing, UI/UX Design, Product Management) and the composite strings that appear verbatim in real profile data (`Git / GitHub`, `Entity Framework Core`, `Generative AI / AI Tools`, `REST API / API Development`). `src/lib/skillVocabulary.ts` binds it to the one existing fold (`skillKey`) and exposes resolve/neighbour lookups; the seed module itself stays dependency-free so a plain Node script can load it. New `npm run check:skills` (in `ci.yml`) fails on a duplicated key, an alias two skills claim, a dangling or self adjacency, a missing EN/TR/DE label, a key that does not survive `skillKey()` and an alias made redundant by the entry's own key or label — none of which TypeScript can see, since every field is a `string`; `npm run test:skill-vocabulary` pins the behaviour (every spelling of React collapsing to one key, adjacency authored one-way coming out symmetric, free text resolving to `null`). Vocabulary only: no schema model, no filter rewiring and no migration of existing free-text skills (#1815/#1819/#1820).

## [0.182.0-beta] - 2026-09-09

_Shipped 2026-09-09 09:51 UTC · commit [ce4810a](https://github.com/21072026/Internship/commit/ce4810afe68d372fc9a65b9f12d6027494fc2d26)_

- **The billable active matched pair now has exactly one definition, plus a nightly usage rollup and video volume counted** (#1750). "How many active pairs does this tenant have?" was answerable three slightly different ways — none of them the published one — and the nearest thing to a billing number (`planGate`'s `status: 'ACTIVE'` count) has no activity requirement, so a dormant pair counted and an invoice built on it would over-bill. The rule is now written once, in dependency-free `src/lib/meteringRules.ts` (unit-tested, `npm run test:metering`) with its queries in `src/lib/metering.ts`: *a relation in ACTIVE state with any logged activity in the calendar month; paused, benched and completed pairs are not counted* — the pricing-page sentence verbatim, as the module's header comment and as an exported constant. Billable states live in ONE allowlist (`BILLABLE_RELATION_STATES`), so a future `PAUSED`/`BENCHED` state is excluded automatically instead of needing seven queries edited; activity is one query with `some` filters over the seven per-relation signals (interaction log, message, meeting, meeting request, stage change, weekly report, goal), and `InteractionLog.date` is used rather than `createdAt` so a backdated log lands in the month it happened. Dormancy is deliberately **not** special-cased — a dormant pair is quiet, and quiet is already excluded. `countAdminSeats()`, `currentPeriod()` and `periodRange()` (half-open UTC month windows) ship from the same file so no caller invents its own month arithmetic; `planGate` now reads its capacity count from there too, with the capacity-vs-usage distinction written down instead of rediscovered. New `UsageRollup` model (`orgId` + `TENANT_MODELS`, unique `(orgId, metric, period)`) is the ledger a usage screen reads, so no request path ever scans seven activity tables; `src/lib/jobs/usageRollup.ts` refreshes the open month and the previous one nightly at 02:40 UTC as a plain handler that ports to the leader-elected scheduler (#1676), registered from `/api/cron/start` next to the other scheduled jobs and runnable on demand via `GET /api/cron?job=usage-rollup`. Idempotent by construction: computed metrics are written absolutely, counters are never recomputed, and `setUsage`/`recordUsage` refuse each other's metrics. Video volume is finally measured at the one room-creation chokepoint (`generateMeetingLink()`) as `VIDEO_ROOM`/`VIDEO_PARTICIPANT` so JaaS MAU exposure is a number rather than a guess — **reported, never gated**: no meeting behaviour, JaaS routing or status code changes, and the counter swallows its own failures. Definition, metric table and the deliberately-not-metered list (mentor seats, mentee seats, programmes): [docs/metering.md](docs/metering.md).
