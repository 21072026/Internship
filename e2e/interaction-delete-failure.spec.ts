...
// something happened that did not. The failure path must say so, and the list
// must not be refetched (a refetch reads as "the delete went through, the row
// is just still here").
test('a rejected interaction delete shows an error, not a "deleted" toast', async ({ page }) => {
  ...
  try {
    ...
    // The 4xx reason is surfaced rather than swallowed…
    await expect(
