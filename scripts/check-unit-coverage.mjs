...
// The size is pinned by EXPECTED_AWAITING so the set cannot grow quietly.
...
function measure(file) {
  ...
  if (!c) return { lines: 0, branch: 0, funcs: 0, seen: false };
  return {
    lines: pct(c.lh, c.lf),
...
for (const [file, reason] of AWAITING_TESTS) {
  ...
  rows.push({
    ...
    floor: null,
    why: reason,
    status: m.seen ? 'NEWLY COVERED' : 'no tests yet',
  });
  if (m.seen) {
    problems.push(
      `${file} now scores ${m.lines.toFixed(2)}% under the node runner — it is no longer ` +
        'untested. Move it from AWAITING_TESTS to FLOORS in scripts/check-unit-coverage.mjs ' +
        `(floor ${Math.min(95, Math.floor(m.lines / 5) * 5)}%, i.e. rounded down to the nearest ` +
        '5 and capped at 95) and drop EXPECTED_AWAITING by one, in this same PR.',
    );
  }
}
...
if (process.env.GITHUB_STEP_SUMMARY) {
  const badge = (status) =>
    status === 'ok' ? '✅ ok' : status === 'no tests yet' ? '⏳ no tests yet' : `❌ ${status}`;
  const md = [
