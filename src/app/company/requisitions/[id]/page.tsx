...
export default function CompanyRequisitionDetailPage() {
  ...
  const requestInterview = async (menteeId: string, payload: { note?: string; proposedSlots?: string[] }) => {
    const res = await fetch('/api/interview-requests', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ requisitionId: id, menteeId, ...payload }) });
  ...
  return <div data-testid="requisition-shortlist"><Link href="/company/requisitions" className="text-sm text-blue-600">← {t.requisitions.title}</Link><h1 className="my-5 text-2xl font-bold text-gray-900 dark:text-gray-100">{requisition.title}</h1>
    ...
    <Card><CardHeader><CardTitle>{text.shortlisted}</CardTitle></CardHeader>{shortlist.length === 0 ? <p className="text-sm text-gray-500">{text.empty}</p> : <div className="space-y-3">{shortlist.map((item) => {
      const request = requests.find((value) => value.menteeId === item.mentee.id);
      return (
        <div key={item.id} data-testid={`shortlist-candidate-${item.mentee.id}`} className="space-y-2 border-b border-gray-100 pb-3 dark:border-gray-800">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <Link href={`/company/candidates/${item.mentee.id}`} className="font-medium text-blue-600">{item.mentee.fullName}</Link>
            <div className="flex flex-wrap items-center gap-2">
              {request ? (
                <>
                  <Badge>{text.statuses[request.status as keyof typeof text.statuses]}</Badge>
                  {request.status === 'APPROVED' && <span className="text-sm text-gray-500">{text.scheduleWithProgram}</span>}
                </>
              ) : (
                <Button size="sm" variant="outline" onClick={() => void removeFromShortlist(item)}>{text.removeFromShortlist}</Button>
              )}
            </div>
          </div>
