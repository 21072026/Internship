  ...
  const tickets = await prisma.supportTicket.findMany({
    ...
    select: {
      ...
      messages: {
        ...
        take: 100,
        select: {
          id: true, body: true, createdAt: true, senderId: true,
...
export async function POST(request: Request) {
  ...
  if (files.length > SUPPORT_ATTACHMENT_MAX_COUNT) {
    return NextResponse.json({ error: `Too many attachments (max ${SUPPORT_ATTACHMENT_MAX_COUNT})` }, { status: 400 });
  }
  ...
  for (const file of files) {
    ...
    if (validationError) {
      ...
      return NextResponse.json({ error: errors[validationError as keyof typeof errors] }, { status: 400 });
    }
  }
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    await tx.supportTicket.update({ where: { id: ticket.id }, data: { updatedAt: new Date() } });
    return { ticketId: ticket.id, messageId: message.id, isNew };
  });
