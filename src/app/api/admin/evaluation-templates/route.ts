...
//
// Both handlers run inside withTenantScope (#1559): `EvaluationTemplate` is
// registered in TENANT_MODELS, so with isolation on the middleware narrows every
// query here to the caller's tenant as well. The explicit `orgId` from
// `resolveOrgId(session)` stays — it is the only filter with the flag off, it is
// the same value the middleware would inject, and the column is REQUIRED, so the
// create below must name it whether or not a context happens to be bound.
