...
// 1200×630 PNG rendered from the same PII-safe field set as the page itself.
//
...
export default async function OpengraphImage({
  ...
  if (!user) {
    ...
    // the image response never reveals whether a given user id exists.
  ...
  // avatarUrl is a relative path (/api/avatar/<id>), which satori cannot
  // fetch — embed the stored bytes as a data URI instead of dialing back
  // into our own HTTP server.
  ...
  if (user.avatarUrl) {
    ...
    if (avatar) {
      avatarSrc = `data:${avatar.contentType};base64,${Buffer.from(avatar.data).toString('base64')}`;
    }
  }
      ...
      >
        ...
        >
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            {bio && (
              <div style={{ fontSize: 20, color: '#374151', marginTop: 12, lineHeight: 1.4 }}>
                {bio}
              </div>
            )}
            {skills.length > 0 && (
