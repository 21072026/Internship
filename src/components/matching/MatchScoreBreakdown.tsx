 ...
 * preview a rule set client-side. The arithmetic that decides *what* it shows
 * (tier, blocking rows, row order, top factor) lives in
 * `src/lib/matching/presentation.ts` and is unit-tested in
 * `e2e/match-score-breakdown.unit.spec.ts`.
 *
 ...
 *   • there is deliberately NO variant that shows the number on its own — even
 *     compact mode always carries a word next to the percentage;
 ...
 *   • a blocking rule is named by what it VIOLATED (`m.violations`), never by
 *     the dimension label: the exclusion dimensions read as the satisfied
 *     condition ("Direct manager"), so using them as the reason would state the
...
const PILL_VARIANT: Record<MatchTier, 'success' | 'warning' | 'default' | 'danger'> = {
  strong: 'success',
  partial: 'warning',
  weak: 'default',
  blocked: 'danger',
...
export function MatchScoreBreakdown({
  ...
  const violationNames = m.violations as Record<string, string>;
  ...
  /**
   * Why a rule blocked. A registry newer than this bundle gets the generic
   * sentence rather than the dimension name, which would read as the *met*
   * condition for the exclusion dimensions.
   */
  const violationLabel = (dimension: string) =>
    violationNames[dimension] ?? m.violationFallback.replace('{name}', dimLabel(dimension));

  const pct = clampPct(score);
  const tier = tierOf(pct, blockedBy);
  const pctLabel = m.percent.replace('{n}', String(pct));
  const srScore = m.srScore.replace('{n}', String(pct));

  if (compact) {
    // One list row at 320px: the pill never shrinks, the label truncates.
    // Blocked shows why it is blocked instead of the top factor — on a narrow
    // row that is the one thing a coordinator has to see. When nothing scored,
    // the tier name stands in: the number must never appear on its own.
    const top = topFactor(breakdown);
    const secondary =
      blockedBy.length > 0
        ? violationLabel(blockedBy[0])
        : top
          ? dimLabel(top.dimension)
          : m.tiers[tier];
    return (
      <span
      ...
      >
        ...
        <span className="sr-only">
          {srScore}
          {blockedBy.length > 0
            ? ` — ${m.tiers.blocked}: ${blockedBy.map(violationLabel).join(', ')}`
            : ''}
        </span>
      </span>
    );
  ...
  const rows = orderRuleRows(breakdown, blockedBy);

  return (
    <Card padding="sm" className={`match-score ${className ?? ''}`} data-testid="match-score" data-tier={tier}>
      <div className="flex items-center gap-4">
        <ScoreRing pct={pct} pctLabel={pctLabel} tier={tier} label={srScore} />
        ...
        <ul className="mt-1 divide-y divide-gray-100" data-testid="match-score-rules">
          {rows.map((entry) => (
            <RuleRow
              ...
              entry={entry}
              blocking={isBlockingRow(entry, blockedBy)}
              label={dimLabel(entry.dimension)}
              violation={violationLabel(entry.dimension)}
...
function RuleRow({
  ...
  return (
    <li
      ...
      data-testid={`match-rule-${entry.dimension}-${entry.kind}`}
    >
      ...
      <span className="min-w-0 flex-1">
        ...
        {blocking && <span className="mt-0.5 block text-xs text-red-700">{violation}</span>}
